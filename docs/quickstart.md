...
For v1.1.2, `ctxpack --version` should print `ctxpack 1.1.2`. If the command is not found, fix your shell or agent `PATH`, or use an absolute binary path in the MCP configuration.
...
ctxpack doctor \
  ...
  --release-manifest /path/to/ctxpack-v1.1.2-aarch64-apple-darwin.manifest.json
```
