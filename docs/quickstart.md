...
For v2.4.6, `ctxhelm --version` should print `ctxhelm 2.4.6`. If the command is not found, fix your shell or agent `PATH`, or use an absolute binary path in the MCP configuration.
...
ctxhelm doctor \
  ...
  --release-manifest /path/to/ctxhelm-v2.4.6-aarch64-apple-darwin.manifest.json
```
