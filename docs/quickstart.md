...
For v1.1.11, `ctxhelm --version` should print `ctxhelm 1.1.11`. If the command is not found, fix your shell or agent `PATH`, or use an absolute binary path in the MCP configuration.
...
ctxhelm doctor \
  ...
  --release-manifest /path/to/ctxhelm-v1.1.11-aarch64-apple-darwin.manifest.json
```
