# Repo Context Packer Product Spec
...
Repo Context Packer is an agent-native, local-first context layer for coding agents.
...
Repo Context Packer is a read-only context broker.
...
-> agent calls ctxpack.prepare_task
-> ctxpack returns target files, related tests, pack URI, and constraints
...
1. Repo initialization
   ...
   - Generate `AGENTS.md` or append a bounded `ctxpack` section safely.
...
5. Early team path
   - Repo-committable config at `.ctxpack/ctxpack.toml`.
...
```text
.ctxpack/ctxpack.toml          repo config, committable when desired
~/.ctxpack/repos/<repo-id>/    local private index/cache by default
```
...
```text
ctxpack.prepare_task
ctxpack.search
ctxpack.related
ctxpack.get_pack
ctxpack.related_tests
ctxpack.current_diff
```
...
ctxpack://pack/{id}/brief
ctxpack://pack/{id}/standard
ctxpack://pack/{id}/deep
ctxpack://file/{path}?lines=42-119
ctxpack://symbol/{fqname}
```
...
```md
For non-trivial code changes, call ctxpack.prepare_task before planning edits.
Use ctxpack for likely target files, related tests, relevant examples,
...
- Generate `.cursor/rules/ctxpack.mdc`.
- Tell Cursor to use its own index for broad awareness and ctxpack for task-specific ranked context, tests, history, and constraints.
...
Integration success means the developer keeps using their existing agent and ctxpack appears only as a context-planning call, not as a separate daily app.
...
- Files ignored by `.gitignore`, `.cursorignore`, or `.ctxpackignore`.
...
- Test relevance: ctxpack recommends at least one plausible targeted validation command when tests exist.
- Privacy trust: ignored, sensitive, and generated files are excluded and pack privacy status is visible.
- Team repeatability: another developer can clone the repo, install ctxpack, and reuse repo config without inheriting private local indexes.

### 9.2 Deferred Evaluation

- Historical PR retrieval eval.
- Agent A/B comparison with and without ctxpack.
...
4. Should `ctxpack.prepare_task` accept active editor/open-file hints through each agent adapter, or only through explicit user-provided paths in MVP?
