# ctxhelm Milestone 3: Safe File Inventory
...
- Respect `.gitignore`, `.ctxhelmignore`, and `.cursorignore`.
...
- Persist inventory JSON under `~/.ctxhelm/repos/<repo-id>/inventory.json`.
- Wire `ctxhelm index` to build and write the inventory.
...
- A temp repo with `.gitignore`, `.ctxhelmignore`, and `.cursorignore` excludes matching files.
...
- `ctxhelm index --repo <repo>` writes `inventory.json` in a private ctxhelm state directory.
