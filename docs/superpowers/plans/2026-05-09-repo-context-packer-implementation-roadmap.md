# Repo Context Packer Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the first agent-native MVP of Repo Context Packer: a local, read-only context broker that exposes compact task context through AGENTS.md, MCP, and thin agent-native adapters.

**Architecture:** Start with a Rust workspace split into `ctxpack-core`, `ctxpack-index`, `ctxpack-compiler`, `ctxpack-mcp`, and a `ctxpack` CLI binary. Keep the default MVP lexical, graph-lite, and local-only; vector retrieval, cloud reranking, SCIP/LSP, and UI remain out of scope.
...
- `ctxpack --help` shows the intended commands.
...
Purpose: Make `ctxpack init` useful in a real repo without indexing yet.

Exit criteria:

- Detect git repo root.
- Create `.ctxpack/ctxpack.toml`.
- Create or patch `AGENTS.md` with a bounded `ctxpack` section.
- Generate `.cursor/rules/ctxpack.mdc`, `.claude/commands/ctxpack-bugfix.md`, and an OpenCode config/instruction snippet only when requested.
...
- Respect `.gitignore`, `.ctxpackignore`, `.cursorignore`, and built-in deny rules.
...
- Persist inventory to local private state under `~/.ctxpack/repos/<repo-id>/`.
...
- Keep `ctxpack search` and MCP `search` output small by default.
...
Purpose: Make ctxpack agent-native.
...
- Add `ctxpack eval traces` to inspect local traces.
...
- `crates/ctxpack`: user-facing CLI binary and command dispatch.
- `crates/ctxpack-core`: shared contracts, repo root detection, privacy policy, and inventory primitives.
- `crates/ctxpack-index`: search/index abstractions and later SQLite/Tantivy implementations.
- `crates/ctxpack-compiler`: task classifier, candidate fusion, context plan, and pack rendering.
- `crates/ctxpack-mcp`: MCP server runtime and mapping between MCP requests and compiler APIs.
- `AGENTS.md`: project instructions for future coding agents working on ctxpack itself.
...
- Do not add autonomous editing behavior to ctxpack.
...
git commit -m "docs: add ctxpack project instructions"
```
...
**Files:**
- Create: `Cargo.toml`
- Create: `crates/ctxpack-core/Cargo.toml`
- Create: `crates/ctxpack-core/src/lib.rs`
- Create: `crates/ctxpack-core/src/contracts.rs`
- Create: `crates/ctxpack-core/src/privacy.rs`
- Create: `crates/ctxpack-core/src/repo.rs`
...
```bash
git add Cargo.toml crates/ctxpack-core
git commit -m "feat: add ctxpack core contracts"
```
...
**Files:**
- Modify: `Cargo.toml`
- Create: `crates/ctxpack-index/Cargo.toml`
- Create: `crates/ctxpack-index/src/lib.rs`
- Create: `crates/ctxpack-compiler/Cargo.toml`
- Create: `crates/ctxpack-compiler/src/lib.rs`
- Create: `crates/ctxpack-mcp/Cargo.toml`
- Create: `crates/ctxpack-mcp/src/lib.rs`
...
[dependencies]
ctxpack-core = { path = "../ctxpack-core" }
```
...
[dependencies]
ctxpack-core = { path = "../ctxpack-core" }
ctxpack-index = { path = "../ctxpack-index" }
...
[dependencies]
ctxpack-compiler = { path = "../ctxpack-compiler" }
ctxpack-core = { path = "../ctxpack-core" }
```
...
```bash
cargo test -p ctxpack-index -p ctxpack-compiler -p ctxpack-mcp
```
...
```bash
git add Cargo.toml crates/ctxpack-index crates/ctxpack-compiler crates/ctxpack-mcp
git commit -m "feat: add ctxpack library crate skeletons"
```
...
**Files:**
- Modify: `Cargo.toml`
- Create: `crates/ctxpack/Cargo.toml`
- Create: `crates/ctxpack/src/main.rs`
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Command::Init => {
            println!("Milestone 0 stub: ctxpack init has no side effects");
        }
        Command::Index => {
...
```bash
cargo run -p ctxpack -- prepare-task "explain the auth flow" --mode explain
```

Expected: JSON output includes `"taskType": "explain"`, `"localOnly": true`, and a `ctxpack://pack/` brief resource URI.
...
```bash
git add Cargo.toml crates/ctxpack
git commit -m "feat: add ctxpack cli skeleton"
```
...
- [ ] Confirm `ctxpack --help` communicates the product surface without implying implemented behavior.
- [ ] Confirm stubs remain read-only and local-only.
- [ ] Confirm follow-up milestones can proceed independently from the scaffold.

## Spec Coverage Check

- Product boundary: covered by read-only stubs, AGENTS.md instructions, and no shell/editor behavior inside ctxpack.
