...
1. Add typed `ContextPack` and `PackSection` contracts in `ctxhelm-core`.
...
4. Add a `ctxhelm get-pack` CLI command with Markdown and JSON output.
...
- Smoke-test `ctxhelm get-pack` on a temporary repository.
