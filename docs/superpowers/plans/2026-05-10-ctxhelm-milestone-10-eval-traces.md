# ctxhelm Milestone 10: Local Evaluation Traces

## Goal

Record local, source-free context-decision metadata so ctxhelm can later evaluate whether its plans and packs helped agents choose the right files and tests.
...
- Add `ctxhelm eval traces` to inspect recent local traces.
- Add `ctxhelm eval checklist` to generate a manual dogfood comparison artifact.
