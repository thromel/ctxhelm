# ctxhelm Milestone 4: Lexical Search

**Goal:** Replace the `ctxhelm search` stub with the first real retrieval path over the safe file inventory.
...
- Load an existing inventory from `~/.ctxhelm/repos/<repo-id>/inventory.json`.
...
- Wire `ctxhelm search <query> --repo <path> --limit <n>`.
