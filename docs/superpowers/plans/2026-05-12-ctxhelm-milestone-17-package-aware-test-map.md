...
Make `ctxhelm://repo/test-map` use the same targeted command inference as `related_tests`, so agents receive accurate validation commands from both tool and resource paths.
...
- No command execution by ctxhelm.
...
- MCP test proves `ctxhelm://repo/test-map` returns package-aware commands.
