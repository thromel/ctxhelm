...
counts, per-task lane metrics, target-read coverage floors, retry-cost fields,
and strict absence of client failures, rate limits, forbidden commands,
...
fresh real-client report when the client version, ctxhelm version, runner,
prompt contract, or task suite changes. JSON output uses
...
aggregate metrics, task-lane checks, boundary checks, and lane quality
summaries.
