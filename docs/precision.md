...
ctxhelm now extracts local symbols with Tree-sitter for TypeScript, JavaScript,
...
ctxhelm can also generate a local source-free precision overlay from those
Tree-sitter symbols:

```bash
ctxhelm precision discover --repo /path/to/repo
...
`.ctxhelm/precision-edges.json`.

For repositories that already run SCIP, LSP, or another code-intelligence indexer, ctxhelm can also import a source-free precision edge overlay:
...
The overlay is written to `.ctxhelm/precision-edges.json`. ctxhelm validates every edge against the safe local inventory before storing it, rejects sensitive/generated/ignored paths, and does not store source snippets, prompt text, secrets, or cloud payloads.
...
Default promotion is evaluated by `ctxhelm eval gate`. If no fresh precision
