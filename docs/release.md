# ctxhelm `ctxhelm` v1.1.9 Release Guide

This document describes the local binary release path for ctxhelm's
`ctxhelm` CLI v1.1.9. The primary user path is a prebuilt archive plus SHA-256
checksums; source builds are fallback paths.

Current public archive release:

```text
https://github.com/thromel/ctxhelm/releases/tag/v1.1.9
```
...
```bash
cargo install --git https://github.com/thromel/ctxhelm --tag v1.1.9 ctxhelm --locked
ctxhelm --version
ctxhelm --help
```
...
From a clean checkout at the v1.1.9 tag, run:
...
```text
dist/ctxhelm-v1.1.9-{target}.tar.gz
dist/ctxhelm-v1.1.9-{target}.manifest.json
dist/ctxhelm-v1.1.9-{target}.audit.json
dist/ctxhelm-v1.1.9-{target}.tar.gz.sha256
dist/sha256sums.txt
```
...
bash scripts/verify-release-archive.sh \
  --archive dist/ctxhelm-v1.1.9-aarch64-apple-darwin.tar.gz \
  --manifest dist/ctxhelm-v1.1.9-aarch64-apple-darwin.manifest.json \
...
bash scripts/verify-public-archive-install.sh \
  ...
  --expected-version "ctxhelm 1.1.9"
```
...
bash scripts/verify-homebrew-tap.sh \
  ...
  --formula ctxhelm \
  --expected-version "ctxhelm 1.1.9" \
  --expected-url https://github.com/thromel/ctxhelm/releases/download/v1.1.9/ctxhelm-v1.1.9-aarch64-apple-darwin.tar.gz \
...
This is the pre-publication blocker for v1.1.9. When `CTXHELM_BIN` is not set, the gate runs `scripts/release-package.sh`, audits the archive, extracts the generated artifact, and uses the extracted `ctxhelm` binary for installed-binary proof.
...
The release gate does not publish, upload, or create GitHub releases, and does not create tags. It does not mutate global agent config and does not run user project tests. Cursor and OpenCode real-client proof is not claimed for v1.1.9.

## Artifact Audit

`scripts/release-package.sh` runs `scripts/audit-release-artifact.sh` immediately after archive creation and before checksum success output. It writes a machine-readable `ctxhelm-v1.1.9-{target}.audit.json` report next to the archive.
...
```bash
bash scripts/audit-release-artifact.sh dist/ctxhelm-v1.1.9-aarch64-apple-darwin.tar.gz
```

## Out of Scope for v1.1

The v1.1.9 release includes a public Apple Silicon Homebrew tap. It does not require crates.io publishing, self-update support, signed installers, cloud telemetry, cloud indexing, cloud embeddings, hosted release services, or global agent config mutation.
