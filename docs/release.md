# ctxhelm `ctxhelm` v2.4.0 Release Guide

This document describes the local binary release path for ctxhelm's
`ctxhelm` CLI v2.4.0. The primary user path is a prebuilt archive plus SHA-256
checksums; source builds are fallback paths.

Current public archive release:

```text
https://github.com/thromel/ctxhelm/releases/tag/v2.4.0
```
...
```bash
tar -xzf ctxhelm-v2.4.0-aarch64-apple-darwin.tar.gz
install -m 0755 ctxhelm-v2.4.0-aarch64-apple-darwin/ctxhelm ~/.local/bin/ctxhelm
ctxhelm --version
ctxhelm --help
ctxhelm doctor --binary "$(command -v ctxhelm)" --release-manifest ctxhelm-v2.4.0-aarch64-apple-darwin.manifest.json
```
...
```bash
cargo install --git https://github.com/thromel/ctxhelm --tag v2.4.0 ctxhelm --locked
ctxhelm --version
ctxhelm --help
```
...
From a clean checkout at the v2.4.0 tag, run:
...
```text
dist/ctxhelm-v2.4.0-{target}.tar.gz
dist/ctxhelm-v2.4.0-{target}.manifest.json
dist/ctxhelm-v2.4.0-{target}.audit.json
dist/ctxhelm-v2.4.0-{target}.tar.gz.sha256
dist/sha256sums.txt
```
...
bash scripts/verify-release-archive.sh \
  --archive dist/ctxhelm-v2.4.0-aarch64-apple-darwin.tar.gz \
  --manifest dist/ctxhelm-v2.4.0-aarch64-apple-darwin.manifest.json \
...
bash scripts/verify-homebrew-tap.sh \
  ...
  --formula ctxhelm \
  --expected-version "ctxhelm 2.4.0" \
  --expected-url https://github.com/thromel/ctxhelm/releases/download/v2.4.0/ctxhelm-v2.4.0-aarch64-apple-darwin.tar.gz \
...
This is the pre-publication blocker for v2.4.0. When `CTXHELM_BIN` is not set, the gate runs `scripts/release-package.sh`, audits the archive, extracts the generated artifact, and uses the extracted `ctxhelm` binary for installed-binary proof.
...
metrics, suite-task checks, task-lane checks, boundary checks, and lane quality
summaries. When `--current-suite` is provided, the checker re-parses the current
suite and validates every saved `tasks[*].taskSha256` plus `tasks[*].targetFiles`
against the current task text and target list. It also validates every task comparison and task lane, so a saved report fails if a nested ctxhelm lane
contains a client failure, rate limit, forbidden command, missing required
ctxhelm call, under-read target, evidence miss, evidence-only target, or
target-read coverage below the release floor even when the aggregate summary
still looks clean. The release gate passes
...
`scripts/release-package.sh` runs `scripts/audit-release-artifact.sh` immediately after archive creation and before checksum success output. It writes a machine-readable `ctxhelm-v2.4.0-{target}.audit.json` report next to the archive.
...
```bash
bash scripts/audit-release-artifact.sh dist/ctxhelm-v2.4.0-aarch64-apple-darwin.tar.gz
```

## Out of Scope for v1.1

The v2.4.0 release includes a public Apple Silicon Homebrew tap. It does not require crates.io publishing, self-update support, signed installers, cloud telemetry, cloud indexing, cloud embeddings, hosted release services, or global agent config mutation.
