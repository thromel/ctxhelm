...
Latest follow-up: Phase 225 expands paired real-agent outcome measurement. `scripts/e2e-agent-run.sh` now compares native baseline, `ctxhelm-plan`, `ctxhelm-brief`, `ctxhelm-standard`, and `ctxhelm-memory`. The standard and memory lanes require valid explicit-repo `prepare_task` plus `get_pack` with `budget = "standard"`, `format = "json"`, and `recordTrace = false`, and the memory lane instructs Claude to use selected memory as guidance while still consuming current files through native reads. `.ctxhelm/e2e/phase225-agent-run-lane-matrix.json` records the current Claude Code `2.1.163` real-client attempt, but all five lanes hit `rate_limited`; the outcome claim remains `insufficient_comparable_lanes` and correctly routes to `retry_real_client_when_available`. This closes the lane-matrix gap but not real-agent outcome lift.
