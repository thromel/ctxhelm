...
Latest follow-up: Phase 153 replaces lexical search's hot path with a query-time Tantivy/BM25 fielded index enriched by exact symbol facets while keeping source-derived inverted index data in memory. Phase 152 makes the paired agent-run outcome harness suite-aware so maintainers can compare native Claude exploration against ctxhelm-assisted lanes across multiple source-free tasks. Phase 151 prepares the `v1.1.12` release line by upgrading the release-artifact workflow so version-tag pushes create/update the GitHub release and upload verified Linux x64, macOS Intel, and macOS Apple Silicon assets, while refreshing required Claude Code workflow proof against `ctxhelm 1.1.12`. Phase 150 added explicit-target packaging and proved the three-target workflow as a non-publishing artifact builder. Phase 149 published and verified the public `v1.1.11` archive and Homebrew tap after Codex diagnostics hardening.

**Phases planned:** Phases 61-65 plus Phase 66-153 production-readiness follow-ups, 7 planned phase files plus measured proof follow-up artifacts
