...
Latest follow-up: Phase 227 hardens the paired-agent report path after a fresh five-lane Claude Code rerun. The rerun still hit `rate_limited` across all lanes, so real-agent outcome lift remains unproven, but Phase 226's retrieval fix held: ctxhelm evidence misses are empty for the assisted lanes. Failed/rate-limited lanes no longer count surfaced targets as `ctxhelmEvidenceOnlyTargets`, and under-read comparisons now require evaluation-eligible ctxhelm lanes. This keeps client-availability blockers separate from retrieval and consumption failures; the remaining recommended R&D action is `retry_real_client_when_available`.
