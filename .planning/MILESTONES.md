...
Latest follow-up: Phase 213 extends source-free R&D action routing into historical eval and product-proof reports. Historical reports now emit `recommendedResearchActions` for candidate-generation gaps, ranked-below-budget misses, progressive next-read recovery, agent-evidence-only recovery, validation-test mapping gaps, protected-evidence misses, and graph edge-budget opportunities. Product-proof reports now aggregate corpus verdicts into fixture/history refresh, runtime, protected-evidence, retrieval/ranking regression, native-baseline, BM25 evidence, or preserve-contract actions.

**Phases planned:** Phases 61-65 plus Phase 66-212 production-readiness follow-ups, 7 planned phase files plus measured proof follow-up artifacts
...
- Source-free recommended R&D actions in paired real-agent, historical eval, and product-proof reports so client availability, retrieval misses, consumption misses, required-call issues, no-lift outcomes, candidate gaps, progressive-read gaps, graph-budget opportunities, runtime gaps, and fixture/evidence gaps route to different next steps.
