...
Latest follow-up: Phase 295 tests the documented `AllMiniLML12V2Q` and `AllMiniLML12V2` local-fastembed model branch on the same VeriSchema older-range `candidate-path-hints` proof. Both variants are hard negatives: `semantic_contribution_no_candidates` across all `19` commits, `semanticCandidateTargetHitCount = 0`, `semanticOnlyTargetHitCount = 0`, no semantic next-read appends, unchanged `local_semantic` Recall@10 `0.29122806`, `semantic_corroborated_reranked targetHitDelta = -1`, and neutral tail-slot reranking. Reject MiniLM12 as a simple model-swap path; corrected Jina remains the useful diagnostic backend, not a default policy.
