...
Latest follow-up: Phase 236 adds real-agent client availability proof after upgrading Codex from the stale Homebrew formula client to Codex CLI `0.137.0`. Codex now produces machine-checkable explicit-repo `prepare_task` and `get_pack` MCP evidence, while Claude Code `2.1.163` remains separately classified as rate-limited (`clientApiErrorStatus = 429`). Current outcome lift is still unproven; the next action is a Codex-backed real-agent outcome matrix rather than another availability retry.
