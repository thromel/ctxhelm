...
Latest follow-up: Phase 226 fixes the Phase 225 secondary ctxhelm-evidence miss for agent-run outcome work. Tasks about paired agent runs, lane matrices, outcomes, and real-client comparison now inject bounded workflow docs (`docs/feedback.md` and `docs/agent-setup.md`) as retrieval candidates. The focused proof shows `prepare-task`, brief packs, and standard packs for `Improve paired agent-run lane matrix` now surface `docs/feedback.md`, and the standard pack includes the feedback snippet. This closes the retrieval/packing gap without claiming real-agent outcome lift; the five-lane Claude matrix still needs a non-rate-limited rerun.
