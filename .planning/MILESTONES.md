...
Latest follow-up: Phase 228 tightens memory-generalization R&D routing. Reports now separate `memoryUniqueNonTargetWithCurrentSupportCount` from unsupported memory noise and expose `supportedMemoryNoiseNeedsReview`. The fresh four-repo semantic-enabled proof measured 12 pairs, kept broad memory lift at `memoryUniqueLiftPairs = 2`, and showed all four raw memory unique non-targets had current support while unsupported memory non-targets stayed at zero. The remaining local memory R&D should inspect supported current-signal pressure rather than demoting memory blindly; real-agent outcome lift still waits on non-rate-limited Claude Code evidence.
