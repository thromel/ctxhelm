...
Latest follow-up: Phase 237 adds and proves a Codex-backed real-agent outcome matrix. Codex CLI `0.137.0` now runs five source-free lanes, satisfies explicit-repo `prepare_task`/`get_pack` calls in all assisted comparable lanes, and produces outcome claim `ctxhelm_improved`: best lane `ctxhelm-memory` improves target coverage by `+0.33`, reduces irrelevant reads by `2`, command executions by `14`, and read-file count by `2`. The remaining R&D action is consumption guidance for doc targets that ctxhelm surfaced but weaker lanes did not read.
