...
Latest follow-up: Phase 161 makes the semantic/precision gate answer whether semantic retrieval is actually contributing beyond lexical: `ctxhelm eval gate` now reports semantic-selected files, target hits, semantic-only target hits, lexical overlap, missed targets, hit rates, and named semantic-only hits. It also accepts `--semantic-provider` so maintainers can run the same gate against `local_hash` or feature-enabled `local_fastembed`. On the clean RefactoringMiner limit-1 proof, `local_hash` selected two semantic files and hit the target but had zero semantic-only target hits, so the gate correctly stayed `hold`. Phase 160 made direct semantic status/search bounded and fixed the concrete `TypeScriptVisitor` semantic ranking miss.

**Phases planned:** Phases 61-65 plus Phase 66-161 production-readiness follow-ups, 7 planned phase files plus measured proof follow-up artifacts
