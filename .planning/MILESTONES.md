...
Latest follow-up: Phase 244 fixes the Codex selected-memory consumption gap exposed on VeriSchema. Earlier memory runs surfaced `schema_agent/core/models.py` as ctxhelm evidence while Codex failed to read it; Phase 244 promotes bounded selected-memory source links into first-read `targetFiles`, adds MCP native-read queue guidance, and carries `CTXHELM_HOME` into the real-client MCP wrapper. The source-free artifact `.ctxhelm/e2e/phase244-agent-run-codex-memory-consumption.json` passes with Codex CLI `0.137.0`, valid explicit-repo `prepare_task`/`get_pack` calls, no forbidden commands, no client failures, no ctxhelm evidence misses, no evidence-only targets, and outcome claim `ctxhelm_improved`: all ctxhelm lanes read `schema_agent/core/models.py`, while best lane `ctxhelm-brief` reduces read files from `18` to `5`, irrelevant reads from `17` to `4`, and command executions by `16`.

**Phases planned:** Phases 61-65 plus Phase 66-225 production-readiness follow-ups, 7 planned phase files plus measured proof follow-up artifacts
