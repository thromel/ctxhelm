...
Latest follow-up: Phase 229 makes supported memory-noise pressure measurable. Reports now expose `memoryUniqueNonTargetCurrentSupportSignalCounts`, `memoryUniqueTargetHitCurrentSupportSignalCounts`, and `supportedMemoryNoiseDominantSignals`. The fresh four-repo semantic-enabled proof measured 12 pairs, kept broad memory lift at `memoryUniqueLiftPairs = 2`, and showed all four raw memory unique non-targets had current support while unsupported memory non-targets stayed at zero. Supported non-target pressure is dominated by `dependency`, `lexical_expansion`, and `symbol`, so the next local memory R&D should tune memory weighting against supported signal pressure; real-agent outcome lift still waits on non-rate-limited Claude Code evidence.
