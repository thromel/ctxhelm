...
Latest follow-up: Phase 159 closes the measured active lexical runtime gap while preserving Phase 158 quality parity: the clean upstream RefactoringMiner proof now matches legacy on Recall@5, Recall@10, and MRR@10 with zero trailing rows, and active backend time is `1688ms` versus legacy `1680ms`. Phase 159 separates shared inventory warmup from backend timing, adds source-safe inventory fingerprints for cache keys, reuses in-process fielded indexes, and stops early for exact-dominant or single-identifier queries. Phase 158 fixed the measured BM25-vs-legacy regression by making exact lexical evidence primary and adding an exact-saturated fast path. Phase 157 added a source-free benchmark corpus health guard and proved the fresh upstream RefactoringMiner fixture is clean before measurement.

**Phases planned:** Phases 61-65 plus Phase 66-159 production-readiness follow-ups, 7 planned phase files plus measured proof follow-up artifacts
