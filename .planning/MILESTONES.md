...
**Latest evidence:** `.planning/e2e/2026-05-31-phase106-real-client-request-evidence.md`

**Delivered through Phase 106:** Codex CLI and Claude Code real-client smoke evidence is now request-auditable without storing raw MCP traffic: evidence files keep compatibility booleans and add source-free request hashes, line counts, explicit repo tool-call counts, sanitized observed tool calls, and request-summary sidecars. Phase 105 also made history-unavailable benchmark repos keep embedded insufficient-evidence reports instead of returning `report: null`.

**Remaining focus:** a hydrated full four-repo proof path and deeper source-free broad-area resources without top-10 churn or runtime-threshold tuning.
