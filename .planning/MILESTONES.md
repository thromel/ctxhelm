...
- ContextMason product branding with `ctxpack` retained as the stable CLI, package, Homebrew formula, MCP namespace, and local state compatibility surface.
- Current public `v1.1.6` archive release with verified GitHub asset digests, install proof, product freshness, refreshed Homebrew tap proof, and optional real-client evidence.
...
- Phase 139 names the product ContextMason while preserving `ctxpack` as the CLI/package/install/MCP compatibility surface and release-gates that brand model through `docs/brand.md`.
