...
Latest follow-up: Phase 337 makes saved agent-run proof audit privacy and boundary fields unambiguous. `scripts/check-agent-run-proof.py` now emits factual `privacyStatus` values from the source report, separate `privacyChecks` pass/fail fields, factual `boundaryStatus` values from the aggregate/comparison report, and existing `boundaryChecks` pass/fail fields. The focused contract test now asserts the Phase 322 summary reports `privacyStatus.sourceTextLogged = false`, source-free privacy checks passed, boundary facts such as `clientFailuresObserved = false`, and boundary checks passed.
