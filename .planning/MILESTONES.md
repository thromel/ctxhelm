...
Latest follow-up: Phase 158 fixes the measured BM25-vs-legacy regression by making exact lexical evidence primary and adding an exact-saturated fast path; the clean upstream RefactoringMiner proof now matches legacy on Recall@5, Recall@10, and MRR@10 with zero trailing rows while improving cold BM25 backend time from Phase 157, though legacy remains faster. Phase 157 adds a source-free benchmark corpus health guard and proves the fresh upstream RefactoringMiner fixture is clean before measurement; the same proof exposed that query-time BM25 trailed legacy on the 20-commit sample. Phase 156 integrates optional BM25-vs-legacy corpus comparison into benchmark and product-proof reports through source-free `lexicalBackendCorpus` repository evidence and `releaseGate.lexicalBackendComparison` aggregate proof. Phase 155 makes BM25-vs-legacy measurement corpus-aware with `ctxhelm eval lexical corpus`, reporting recall, MRR, overlap, top-path churn, win/tie counts, and backend runtime across historical commit tasks.

**Phases planned:** Phases 61-65 plus Phase 66-158 production-readiness follow-ups, 7 planned phase files plus measured proof follow-up artifacts
