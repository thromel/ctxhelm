...
Latest follow-up: Phase 212 routes paired real-agent evidence into source-free R&D actions. Single-run reports and suite aggregates now include `recommendedResearchActions`, distinguishing client availability retries, retrieval/query fixes, agent-consumption guidance, required-call guidance, native-baseline analysis, and stable-contract preservation. The latest real Claude Code probe remains rate-limited and now recommends `retry_real_client_when_available` only, while skipped non-real reports recommend collecting real-client evidence instead of falsely blaming prompt guidance.
...
- Source-free recommended R&D actions in paired real-agent reports so client availability, retrieval misses, consumption misses, required-call issues, and no-lift outcomes route to different next steps.
