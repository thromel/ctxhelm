...
Latest follow-up: Phase 233 clears the unsupported memory precision issue found by Phase 232. Ranking now skips uncorroborated memory-only source rescue when native related-test evidence already exists, while preserving the existing memory-only rescue path when memory is the only evidence. The focused express rerun drops unsupported memory non-targets from `1` to `0`; the fresh six-repo semantic-enabled proof still measures 30 pairs across 30 distinct target files, preserves `memoryUniqueLiftPairs = 2`, reduces `memoryUniqueNonTargetCount` from `2` to `1`, and reports `unsupportedMemoryNoiseRepositoryCount = 0`. The next local memory R&D is strong-signal overlap inspection for the remaining co-change/dependency-supported express non-target. Real-agent outcome lift still waits on non-rate-limited Claude Code evidence.
