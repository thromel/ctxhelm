...
Latest follow-up: Phase 132 added a deeper Claude Code workflow eval, recording that Claude Code `2.1.159` produces explicit-repo `prepare_task` and `get_pack` evidence through MCP while committing only source-free hashes and sanitized request metadata.
