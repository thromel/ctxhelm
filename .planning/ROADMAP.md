...
Phase 257 hardens the eval-only local metadata reranker after the semantic rich-document rejection. The reranked variant now preserves protected source evidence from the default top-K ranking and keeps the existing validation-test reserve. Fresh local source-free gates report zero named regressions on RefactoringMiner and ctxhelm. RefactoringMiner is safe but neutral at Recall@10 `0.5383333`, while ctxhelm improves from `0.30929655` to `0.5414069`; the gate remains `hold`, so this is a safety/eval promotion rather than a default reranker promotion.

Phase 258 makes that reranker behavior inspectable enough for routing R&D. Semantic/precision gate reports now include `rerankerContribution`, with improved/regressed/neutral commit counts, target-hit delta, reranker-only target hits, default-only target hits, protected miss-rate deltas, and target-churn examples. The fresh reports show RefactoringMiner is neutral while ctxhelm has `targetHitDelta = +14` and two explicit default-only `.planning/STATE.md` churn cases, so the next step is query-family routing or learned fusion with churn constraints rather than unconditional reranker defaulting.

Phase 259 closes the runtime gap for opt-in reranking. The policy-enabled local metadata reranker now preserves a protected source floor for anchor/current-diff/lexical/symbol evidence before metadata reordering, while keeping tests outside that source floor. The planner-level policy test proves `.ctxhelm/provider-policy.json` can enable the reranker through `prepare_context_plan`, and fresh gates remain regression-free: RefactoringMiner is neutral, ctxhelm has `targetHitDelta = +16`, and the known `.planning/STATE.md` churn remains explicit. This makes routed/learned reranker experiments safer without turning reranking on by default.

Phase 260 makes reranker routing evidence query-family aware. `rerankerContribution` now includes `queryFamilyContributions` with per-family target-hit deltas, default-only churn, examples, and a routing recommendation. Fresh local source-free gates keep RefactoringMiner neutral by family, while ctxhelm exposes two clean route candidates (`commit_clue`, `symbol_identifier`) and one `domain_phrase` family held for churn. This moves the semantic/reranker R&D path from aggregate lift toward measurable routing constraints without promoting reranking globally.

Phase 261 turns those family labels into an eval-only routed reranker variant. `query_family_routed_reranked` applies local metadata reranking only for conservative route-safe families and keeps default ranking elsewhere. Fresh gates show the routed policy is neutral on RefactoringMiner and improves ctxhelm from default Recall@10 `0.3036905` to `0.48845237` with zero default-only target churn, while full reranking reaches `0.6342858` but still churns two default target hits. This makes routed reranking a measurable policy candidate, still not a default runtime behavior.

Phase 262 broadens that routed-policy proof and narrows the route set. The four-repo gate rejected routing `symbol_identifier` because ReAgent lost 5 target hits under that family, so the eval-only routed variant now routes only `commit_clue`. Fresh artifacts show zero routed regressions and zero routed default-only churn on RefactoringMiner, ctxhelm, ReAgent, and VeriSchema. ctxhelm keeps a smaller but clean lift (`0.3536905 -> 0.5005952`, `targetHitDelta = +9`), while the other three repos are neutral under routed reranking. Full metadata reranking remains unsafe on ReAgent and VeriSchema.

Phase 263 exposes the narrowed route as opt-in runtime behavior. Provider policy now accepts `enableQueryFamilyRoutedReranker`; when enabled, planner reports expose a local metadata-only `local_metadata_routed` reranking decision and apply reranking only for the measured `commit_clue` family. Unproven families such as `symbol_identifier` keep default ranking and emit a held diagnostic. Defaults remain unchanged: no semantic or reranker path is promoted globally.

Phase 264 makes semantic contribution query-family aware and refreshes the real-client availability state. Claude Code remains rate-limited, so no fresh Claude paired outcome claim is made. Semantic gate reports now include `semanticContribution.queryFamilyContributions` and diagnostics for route candidates, mixed holds, and noise holds. Fresh four-repo local-fastembed gates show selective candidates but no default promotion: VeriSchema `domain_phrase` and ctxhelm `broad_scope` have semantic-only target hits without semantic-only non-targets, while ReAgent families are noise holds and RefactoringMiner `symbol_identifier` is mixed.

Phase 265 stress-tests those semantic family candidates on a broader `limit 20` slice and adds commit-level stability counters. `semanticContribution.queryFamilyContributions` now reports target-hit commits, non-target commits, clean target-only commits, mixed commits, noise-only commits, and commit rates, plus `semantic_query_family_unstable_hold` when a family has both clean target-only commits and same-family noise. The broader four-repo proof rejects coarse semantic family routing for now: ctxhelm `broad_scope` and VeriSchema `domain_phrase` both become unstable, ReAgent stays pure semantic noise, and RefactoringMiner `symbol_identifier` keeps small lift with heavy noise. Semantic remains opt-in until stricter within-family gates or better local query/document construction pass the same stability check.

Phase 266 adds support profiles for semantic-only files and rejects the next simple routing rule. The gate now records whether semantic-only targets and semantic-only non-targets also have non-semantic source-free support, grouped by support family. Fresh four-repo `limit 20` proof shows generic corroboration is not enough: semantic-only non-targets are also corroborated, and several query families emit supported mixed/noise holds. The next semantic experiment should target support-family-specific constraints or better local semantic query/document construction rather than routing on "semantic plus any support."

Phase 267 makes those support-family constraints explicit as diagnostics. Gate reports now classify each semantic support profile as a route candidate, mixed hold, or noise hold. Fresh four-repo `limit 20` proof finds sparse route candidates but many holds: ctxhelm has one candidate, ReAgent has none and eight noise holds, RefactoringMiner has no candidates and mixed/noise holds, and VeriSchema has three candidates but remains blocked with thirteen noise holds. This narrows the next semantic R&D step to an eval-only support-profile-routed variant, not runtime policy exposure.
...
Phase 271 adds `semanticCorroboratedRerankerContribution` to test that narrower
route. The new per-query-family diagnostics show no safe coarse query-family
separator: RefactoringMiner has clean route candidates for `domain_phrase`,
`symbol_identifier`, and `commit_clue`, but `domain_phrase` regresses or churns
on ctxhelm, ReAgent, and VeriSchema, while `symbol_identifier` churns ctxhelm
and regresses ReAgent. Query-family-only routing is rejected; the next semantic
R&D should inspect corpus-shape/path-role constraints for the RefactoringMiner
lift and separate query/document construction for VeriSchema.

Phase 272 adds source-free path-family contribution diagnostics for the same
semantic-corroborated variant. RefactoringMiner's lift is clean across docs,
Gradle config, Java source, and Java tests, but broad path-family routing still
does not generalize: ctxhelm churns docs/planning/Rust source, ReAgent loses
planning/scripts, and VeriSchema loses Python source. This rejects broad
path-family routing and narrows the remaining semantic work to corpus-shape
constraints such as Java/MCP/package coherence, or to VeriSchema-specific
query/document construction.

Phase 273 probes the simplest VeriSchema candidate-generation hypothesis by
raising `CTXHELM_FASTEMBED_DOCUMENT_LIMIT` to `256`. The metrics are unchanged:
local semantic remains `0.39382353`, semantic-corroborated remains
`0.36960787`, semantic candidate target hits remain `11`, and Python source
still blocks at `-10`. This rejects larger local-fastembed prefilter caps as
the next fix and keeps the remaining VeriSchema work focused on query/document
construction or Python package/module facets.

Phase 274 tests and rejects path-derived Python package/module facets. The
temporary source-free facets increased semantic candidate target hits from `11`
to `12`, but semantic-corroborated recall fell from `0.36960787` to
`0.32294118`, semantic-only non-targets increased, and Python source remained
churned. The code was reverted; future VeriSchema work should focus on task
...
Phase 279 promotes task-mentioned commit changed paths into prepare-task
anchors. Public reproduction tasks often name an upstream commit SHA but not the
changed files; the planner now recognizes explicit `commit` prompts containing
7-40 hex revision tokens, reads only local `git diff-tree --name-status -z`
metadata for those revisions, filters the paths through the safe inventory, and
adds the surviving paths as explicit path facets. The behavior is source-free,
diagnostic-backed, and bounded by the normal prepare-task target limit, so it
improves commit-reproduction recall without adding autonomous editing or source
content logging.

Phase 280 adds a durable source-free learned semantic policy artifact to the
semantic gate. `learnedSemanticPolicy` records a deterministic policy id,
source variant, source eval range, ranking budget, profile rows, staleness
status, holdout status, and a repeated support threshold
(`minimumSupportCommitCount = 2`). The four-repo local-fastembed proof preserves
Phase 278's no-regress learned-profile metrics while exporting only one
eligible durable profile: RefactoringMiner `symbol_identifier/docs`, observed
in two commits. Every artifact sets `runtimePromotable = false`, so the next
...
Phase 282 checks the simplest broader-corpus hypothesis by rerunning the same
source-free learned-policy holdout at `limit 40`. The wider slice still has
`appliedCommitCount = 0` on RefactoringMiner, ctxhelm, ReAgent, and VeriSchema.
RefactoringMiner keeps learned-profile lift in the full snapshot, but its only
eligible policy row is still `symbol_identifier/docs` with exactly two
observations, so leave-one-out support remains below threshold on the held-out
...
Phase 287 tests the first query-construction follow-up after the profile-key
relaxation was rejected. `ctxhelm eval history`, `ctxhelm eval gate`, and
`ctxhelm eval learned-policy-train-test` now accept the eval-only
`--semantic-query-mode source-role-hints` flag, which appends source-free
coding-role words and dominant source languages to semantic queries while
leaving runtime/default query construction at `plain`. The targeted VeriSchema
older-range proof rejects this generic source-role hinting: candidate targets
increase only `13 -> 14`, candidate misses increase `4 -> 6`, selected semantic
targets drop `9 -> 8`, semantic-only targets drop `2 -> 0`, and
`semantic_corroborated_reranked` still has `targetHitDelta = -1` with one
...
Phase 291 tests a narrower path-neighborhood query construction idea. The
eval-only `--semantic-query-mode candidate-sibling-path-hints` mode appends
bounded same-directory and mirrored-test aliases near top lexical candidates.
The same VeriSchema older-range proof rejects it: semantic candidate misses
worsen from `3 -> 4`, selected semantic target hits fall from `11 -> 10`,
semantic-only non-targets only fall from `16 -> 15`, the semantic-corroborated
...
Phase 294 adds `semanticNextReadContribution` to test semantic as bounded
next-read guidance after protecting the full default top K. The field reports
up to two semantic-ranked files per commit that are not already in the default
top K, grouped by query, path, and support families. On the same VeriSchema
older-range Jina candidate-path proof, semantic appends only one next-read path
and it is a non-target (`tests/core/test_state_validator.py`), so the gate emits
`semantic_next_read_noise_hold`. This rejects semantic next-read promotion for
the current setup while keeping the source-free diagnostic for future model or
...
Phase 298 adds that source-free predictor as the eval-only
`semantic_supported_shape_tail_slot_reranked` variant. Historical commit rows
now expose `supportedSemanticCandidateProfilesAt10`, and the variant uses only
those generated supported semantic candidate profiles to insert the measured
`symbol_identifier` / `python_source` / `dependency_co_change` shape into
protected tail slots. On the same VeriSchema Jina proof it matches the oracle's
clean result: `schema_agent/core/state.py`, target hits `13 -> 14`, file
...
Phase 302 adds a strict held-out `retention-separator-train-test` diagnostic.
Recent-to-older train/test reports across ctxhelm, ReAgent, RefactoringMiner,
and VeriSchema find 117 train families and 377 dropped test profiles, but zero
...
Phase 313 records the R&D completion gate. The product is strong enough to keep
shipping and has current Codex/memory/release proof, but the active R&D goal is
not complete until semantic is explicitly closed or promoted, and Claude is
either comparable again or formally classified as an external availability
blocker in the final audit.

Phase 314 applies that gate and closes the active R&D scope. Semantic/default
promotion stays diagnostic-only after the recorded Phase 252-303 same-family
negative results, with future semantic work treated as new R&D unless it brings
a materially different source-free feature family or corpus. Claude
cross-agent replication is classified as an external availability blocker based
on current source-free rate-limit/client-failure evidence with zero comparable
lanes.

Phase 315 starts the Agent Outcome Reliability milestone. The first step keeps
the distribution metadata smoke temp-path clean during release-gate runs,
preserves explicit opt-in for refreshing the tracked distribution metadata
artifact, and adds release-gate worktree cleanliness checks after smokes and at
final completion.
