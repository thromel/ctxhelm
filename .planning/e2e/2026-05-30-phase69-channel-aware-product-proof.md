...
Fix the product-proof gate so it reflects ctxpack's two output channels:
...
- Root governance docs get a small selection floor only when `.planning` governance docs are present, avoiding generic README/doc displacement in non-ctxpack repositories.
...
```bash
cargo run -p ctxpack -- eval proof --config .ctxpack/e2e/phase62-default-config.json --format json > .ctxpack/e2e/phase69-channel-scoped-governance-proof.json
python3 scripts/check-product-proof.py .ctxpack/e2e/phase69-channel-scoped-governance-proof.json
```
...
| ctxpack | beat | 0.3333 | 0.2857 | +0.0476 | 0.3169 | 0.2773 | 1.0000 | 0.2200 |
