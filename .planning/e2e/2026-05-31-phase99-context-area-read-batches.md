...
- Added source-free `roleBuckets` to dynamic
  `ctxhelm://repo/context-area/{encoded-area}` resources.
...
cargo test -p ctxhelm-mcp resource_read_returns_repo_summary_and_test_map -- --nocapture
```
...
cargo run --release -p ctxhelm -- eval proof \
  ...
  --format json > .ctxhelm/e2e/phase99-context-area-read-batches-proof.json

python3 scripts/check-product-proof.py \
  .ctxhelm/e2e/phase99-context-area-read-batches-proof.json
```
