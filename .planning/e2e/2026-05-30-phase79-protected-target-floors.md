...
cargo run -p ctxpack -- eval proof \
  ...
  --format json > /tmp/ctxpack-phase79-required-proof.json
python3 scripts/check-product-proof.py /tmp/ctxpack-phase79-required-proof.json
```

Committed artifact:

- `.ctxpack/e2e/phase79-protected-target-floors-proof.json`
...
| ctxpack | `beat` | 0.541 | 0.351 | 0.000 | 0.042 |

Gate decision: `promote`.

RefactoringMiner protected retrieval-target misses drop to zero. The required
ctxpack slice still has one protected source-symbol target below budget:
...
cargo run -p ctxpack -- eval proof \
  ...
  --format json > /tmp/ctxpack-phase79-broader-proof.json
python3 scripts/check-product-proof.py /tmp/ctxpack-phase79-broader-proof.json
```

Committed artifact:

- `.ctxpack/e2e/phase79-broader-protected-target-floors-proof.json`
...
| ctxpack | `beat` | 0.389 | 0.306 | 0.000 | 0.100 |
| ReAgent | `beat` | 1.000 | 0.571 | 1.000 | 0.000 |
| VeriSchema | `beat` | 0.192 | 0.082 | 1.000 | 0.000 |

Gate decision: `promote`.

VeriSchema protected retrieval-target misses drop to zero. The broader ctxpack
...
```bash
cargo fmt --check
cargo test -p ctxpack-compiler selection_reserves_symbol_floor_when_ordinary_lexical_files_fill_budget -- --nocapture
cargo test -p ctxpack-compiler selection_reserves_exact_config_when_scripts_and_docs_dominate -- --nocapture
cargo test -p ctxpack-compiler selection_treats_agent_setup_as_governance_context -- --nocapture
cargo test -p ctxpack-compiler selection_defers_archive_artifacts_in_final_fill -- --nocapture
cargo test -p ctxpack-compiler selection_reserves_symbol_floor_when_archive_docs_fill_budget -- --nocapture
