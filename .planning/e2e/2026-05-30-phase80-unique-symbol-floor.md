...
Close the remaining ctxhelm protected source-symbol target misses without
...
required and broader ctxhelm slices.
...
cargo run -p ctxhelm -- eval proof \
  ...
  --format json > /tmp/ctxhelm-phase80-required-proof.json
python3 scripts/check-product-proof.py /tmp/ctxhelm-phase80-required-proof.json
```

Committed artifact:

- `.ctxhelm/e2e/phase80-unique-symbol-floor-proof.json`
...
| ctxhelm | `beat` | 0.589 | 0.397 | 0.000 | 0.000 |

Gate decision: `promote`.

The previous required ctxhelm miss,
`crates/ctxhelm-compiler/src/planning.rs`, is now selected inside the standard
budget.
...
cargo run -p ctxhelm -- eval proof \
  ...
  --format json > /tmp/ctxhelm-phase80-broader-proof.json
python3 scripts/check-product-proof.py /tmp/ctxhelm-phase80-broader-proof.json
```

Committed artifact:

- `.ctxhelm/e2e/phase80-broader-unique-symbol-floor-proof.json`
...
The previous broader ctxhelm miss,
`crates/ctxhelm-index/src/related_tests.rs`, is now selected inside the standard
budget.
...
```bash
cargo fmt --check
cargo test -p ctxhelm-compiler selection_counts_unique_source_symbol_floor_additions -- --nocapture
cargo test -p ctxhelm-compiler selection_reserves_symbol_floor -- --nocapture
