...
```bash
cargo test -p ctxhelm-compiler historical_eval_separates_new_files_from_retrievable_context_targets -- --nocapture
...
produced `.ctxhelm/e2e/phase67-retrievable-target-proof.json`.
...
| ctxhelm | `ctxhelm_default` | `match` | 0.2277 | 0.2326 | -0.0049 | 1.0000 | 0.1500 |

## Decision

Phase 67 improves eval fidelity and removes false misses for unretrievable newly-added files. It does not make ctxhelm production-ready. The release proof still blocks default promotion because every corpus does not beat lexical.
