...
- Corpus ID: `ctxpack-v2.3-refactoringminer-20-commit-clean-e2e`
...
- Fixed corpus ID: `ctxpack-v2.3-refactoringminer-20-commit-clean-e2e`
...
- Neutral: current ctxpack Recall@10 is within 1 percentage point of the locked baseline.
...
- `CTXPACK_BIN=target/debug/ctxpack bash scripts/smoke-v23-eval.sh`: passed.
- `CTXPACK_BIN=target/debug/ctxpack CTXPACK_SMOKE_REPO=$PWD bash scripts/smoke-historical-eval.sh`: passed for ctxpack's own 3-commit smoke corpus.
- RefactoringMiner 20-commit clean-worktree benchmark: passed with no quality regression versus locked baseline.
- `CARGO_TARGET_DIR=/tmp/ctxpack-release-gate-target CARGO_INCREMENTAL=0 CTXPACK_ALLOW_DIRTY=1 CTXPACK_SKIP_REAL_CLIENT=1 bash scripts/release-gate.sh`: passed.
...
- Verified the exact failing Claude wrapper protocol case now passes: `CTXPACK_SMOKE_TASK="e2e claude wrapper smoke"` with `CTXPACK_SMOKE_PATH="crates/ctxpack/src/main.rs"`.
