...
- Historical parent snapshots now get a source-free `.ctxpack/eval-history.json`
...
This keeps ctxpack local-first and source-safe: the sidecar contains no source
...
cargo run -p ctxpack -- eval proof \
  ...
  --format json > /tmp/ctxpack-phase75-two-repo-proof.json
python3 scripts/check-product-proof.py /tmp/ctxpack-phase75-two-repo-proof.json
```

Committed artifact:

- `.ctxpack/e2e/phase75-parent-history-test-reserve-proof.json`
...
| ctxpack | `beat` | 0.478 | 0.391 | 1.000 | 0.074 |

Gate decision: `promote`.

Compared with Phase 74, ctxpack target protected miss-rate improved from 0.133
...
cargo run -p ctxpack -- eval proof \
  ...
  --format json > /tmp/ctxpack-phase75-broader-proof.json
```

Committed artifact:

- `.ctxpack/e2e/phase75-broader-parent-history-test-reserve-proof.json`
...
```bash
cargo test -p ctxpack-index co_change_hints_use_eval_history_sidecar_when_snapshot_has_no_git -- --nocapture
