...
cargo run -p ctxhelm -- eval proof \
  ...
  --format json > /tmp/ctxhelm-phase74-two-repo-proof.json
python3 scripts/check-product-proof.py /tmp/ctxhelm-phase74-two-repo-proof.json
```

Committed artifact:

- `.ctxhelm/e2e/phase74-protected-evidence-diagnostics-proof.json`
...
| ctxhelm | `beat` | 0.500 | 0.429 | 1.000 | 0.143 | 0.133 |
...
- ctxhelm: `crates/ctxhelm-index/src/lib.rs`,
  `crates/ctxhelm-compiler/src/ranking.rs`, `.planning/STATE.md`, and
...
cargo run -p ctxhelm -- eval proof \
  ...
  --format json > /tmp/ctxhelm-phase74-broader-proof.json
```

Committed artifact:

- `.ctxhelm/e2e/phase74-broader-protected-evidence-diagnostics-proof.json`
...
| ctxhelm | `beat` | 0.361 | 0.306 | 0.000 | 0.163 | 0.100 |
...
- VeriSchema and ctxhelm still have protected retrieval-target misses.
