...
```bash
cargo test -p ctxhelm-index semantic --features local-embeddings -- --nocapture
...
- model cache defaults to repo `.ctxhelm/cache/fastembed` inside a git repo, otherwise `CTXHELM_HOME/cache/fastembed`
- override supported through `CTXHELM_FASTEMBED_CACHE_DIR`
- query-time source-free vector cache is bounded in process
- expensive model candidate prefilter defaults to `128`
- override supported through `CTXHELM_FASTEMBED_DOCUMENT_LIMIT`
...
| Variant | RefactoringMiner runtime | ctxhelm runtime |
| --- | ---: | ---: |
