...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   target tests. The remaining semantic path should not add another handwritten
   route without explicit default target-test/source preservation.
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   Phase 275 rejects simple query/path-shape insertion. The next semantic step
   is commit-level failure analysis for default target-test/source preservation
   or a learned/listwise budget allocation experiment with explicit no-regress
   constraints. Increasing the local-fastembed document cap and adding broad
   Python path metadata are both already rejected.
...
rule, Phase 272 rejects broad path-family routing, and Phase 275 rejects simple
query/path-shape insertion after it loses default target tests. The remaining
semantic R&D path is now target-preserving budget constraints or learned/listwise
routing, plus separate VeriSchema query/document-construction work.
