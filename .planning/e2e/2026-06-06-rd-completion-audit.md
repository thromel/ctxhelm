...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   The next semantic work should treat corrected Jina as a diagnostic backend
   and seek a larger training surface or different source-free features, not
   more single-query expansion, MiniLM model swaps, next-read promotion, broad
   document expansion, supported-shape promotion, same-proof relaxation, or
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   train families, zero eligible test overlap, and zero applications. The next
   semantic step needs more training surface or different source-free features,
   not same-proof relaxation or runtime promotion.
...
so the remaining semantic R&D path needs more training surface or different
source-free separator features, not more same-range doubling, not coarse
profile-key relaxation inside the same proof, not generic role words, not
current cross-repo aggregation, not candidate-path hints, candidate-sibling
hints, generic concept-term hints, tail-slot reranking, semantic next-read
promotion, MiniLM12 model swaps, supported-shape promotion, or Jina as
runtime/default policy, and not runtime/default promotion.
