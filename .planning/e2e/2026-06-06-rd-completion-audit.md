...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   first query-family routing view and Phase 261 evaluates a conservative routed
   policy. The routed policy removes measured ctxhelm default-only churn while
   preserving part of the full reranker lift, but the evidence is still only two
   corpora. The right next work is broadening the routed policy proof across
   more repos and then deciding whether it belongs in provider policy as opt-in
   runtime behavior, not unconditional semantic or reranker promotion.
