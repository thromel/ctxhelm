...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   first query-family routing view and shows route candidates only for specific
   families; `domain_phrase` remains held for churn. The right next work is
   using those family labels to evaluate a gated route/learned-fusion policy
   across more corpora, not unconditional semantic or reranker promotion.
