...
artifacts after Phase 281. This audit is not a release claim by itself; it
...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   held-out commits across all four repos. Promotion still needs broader
   training/holdout evidence where exported profiles can apply outside the
   training snapshot.
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   current slice is too sparse for repeated-support application. The next
   semantic step is broader training/holdout corpus construction or separate
   revision slices, not runtime promotion. Increasing the local-fastembed
   document cap, adding broad Python path metadata, and adding another
   hand-written route are already rejected paths.
...
`appliedCommitCount = 0` everywhere with no regressions. The remaining semantic
R&D path is broader training/holdout evidence for learned policies, plus
separate VeriSchema query/document-construction work.
