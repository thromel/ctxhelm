...
artifacts after Phase 276. This audit is not a release claim by itself; it
...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   enter top-K while default target tests/source files are lost. The remaining
   semantic path should not add another handwritten route without explicit
   default target-test/source preservation.
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   query-family-only route, Phase 272 rejects broad path-family routing, and
   Phase 275 rejects simple query/path-shape insertion. Phase 276 identifies
   the displacement families that any next experiment must guard against. The
   next semantic step is target-preserving budget allocation or a learned/listwise
   experiment with explicit no-regress constraints. Increasing the local-fastembed
   document cap and adding broad Python path metadata are both already rejected.
...
query/path-shape insertion after it loses default target tests, and Phase 276
shows the displacement pressure behind those losses. The remaining semantic R&D
path is now target-preserving budget constraints or learned/listwise routing,
plus separate VeriSchema query/document-construction work.
