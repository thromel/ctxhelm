...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   targeted slice. The next semantic work should treat corrected Jina as a
   diagnostic backend and broaden that exact predictor across ranges/repos, or
   build a much narrower learned separator, not more single-query expansion,
   MiniLM model swaps, next-read promotion, broad document expansion, or default
   model promotion.
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   `symbol_identifier` / `python_source` / `dependency_co_change` supported
   shape predictor on the targeted slice. The next semantic step is broader
   validation of that supported-shape predictor, or a materially narrower
   learned separator, not runtime promotion.
...
source-free supported-shape predictor on the targeted slice. The remaining
semantic R&D path is broader validation of that supported-shape predictor or a
much narrower learned separator, not more same-range doubling, not coarse profile-key
