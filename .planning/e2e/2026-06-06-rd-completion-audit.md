...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   Phase 270 rejects the first global fusion rule, and Phase 271 rejects the
   query-family-only route. The next semantic step is corpus-shape/path-role
   diagnostics for the RefactoringMiner lift plus separate query/document
   construction work for VeriSchema.
...
rule, leaving corpus-shape/path-role constraints and VeriSchema
query/document-construction work as the remaining semantic R&D path.
