...
artifacts after Phase 282. This audit is not a release claim by itself; it
...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   broadening is not enough. Promotion needs explicit train/test revision ranges
   or cross-repo profile aggregation where exported profiles can apply outside
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   rules out the simplest same-gate widening fix at `limit 40`. The next
   semantic step is explicit train/test revision-slice support or cross-repo
   profile aggregation, not runtime promotion. Increasing the local-fastembed
   document cap, adding broad Python path metadata, adding another hand-written
   route, and merely widening the same gate are already rejected paths.
...
same result at `limit 40`, so the remaining semantic R&D path is explicit
train/test revision-slice support or cross-repo learned-policy aggregation,
