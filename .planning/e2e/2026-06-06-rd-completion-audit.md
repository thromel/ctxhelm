...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   is also held as diagnostic-only. The next semantic work should treat
   corrected Jina as a diagnostic backend and focus on materially different
   source-free document/model construction or a much narrower learned separator,
   not more single-query expansion, next-read promotion, or default model
   promotion.
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   non-target appended path. The next semantic step is
   ...
   next-read promotion from the current proof, and Jina as the default
   local-fastembed model are already rejected paths. Corrected Jina remains
   useful diagnostic evidence for candidate quality.
...
path is a non-target. The remaining semantic R&D path is
...
tail-slot reranking, semantic next-read promotion, or Jina as runtime/default
