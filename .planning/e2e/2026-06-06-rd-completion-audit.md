...
same VeriSchema older-range `candidate-path-hints` proof. Both variants emit
`semantic_contribution_no_candidates` across all `19` commits, produce
`semanticCandidateTargetHitCount = 0`, leave `local_semantic` Recall@10
unchanged at `0.29122806`, keep the known semantic-corroborated
`targetHitDelta = -1`, and leave tail-slot reranking neutral. This rejects
MiniLM12 as a simple model-swap branch.
...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   variants and rejects them because both produce no semantic candidates. The
   next semantic work should treat corrected Jina as a diagnostic backend and
   focus on materially different source-free document construction or a much
   narrower learned separator, not more single-query expansion, MiniLM model
   swaps, next-read promotion, or default model promotion.
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   branch after both variants produce no semantic candidates. The next semantic
   step is
   ...
   and Jina as the default local-fastembed model are already rejected paths.
   Corrected Jina remains useful diagnostic evidence for candidate quality.
...
both produce no semantic candidates. The remaining semantic R&D path is
fusion/selection constraints over improved candidates or a much narrower
