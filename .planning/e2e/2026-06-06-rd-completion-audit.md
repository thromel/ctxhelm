...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   query-family-only route, and Phase 272 rejects broad path-family routing.
   The next semantic step is narrower corpus-shape diagnostics, such as
   Java/MCP/package coherence, plus separate query/document construction work
   for VeriSchema. Increasing the local-fastembed document cap and adding broad
...
rule, and Phase 272 rejects broad path-family routing. The remaining semantic
R&D path is now narrower corpus-shape constraints plus VeriSchema
