...
4. Run the next semantic experiment against the Phase 266 support profiles:
   support-family-specific constraints or alternate local query/document
   construction must beat the generic-corroboration hold before any runtime
   semantic routing is added.
...
266 rejects generic corroboration as a sufficient separator. Phase 253 reduces
memory risk by proving cross-repo Codex target-consumption lift. Phase 255
further reduces memory risk by preserving that target consumption while
improving irrelevant reads in all three measured memory pairs.
