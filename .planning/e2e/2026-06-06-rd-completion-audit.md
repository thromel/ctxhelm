...
artifacts after Phase 279. This audit is not a release claim by itself; it
...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   ctxhelm, ReAgent, and VeriSchema remain exactly neutral. This is promising
   R&D evidence, not runtime/default completion. Promotion still needs a
   durable learned-policy artifact, a staleness contract, minimum support
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   three repos while lifting RefactoringMiner, so the next semantic step is a
   durable learned-policy artifact with staleness handling, support thresholds,
   and cross-repo holdout gates. Increasing the local-fastembed document cap,
   adding broad Python path metadata, and adding another hand-written route are
...
budget rule as a general solution, and Phase 278 shows a more credible
learned-profile direction with no regressions across the four measured repos.
The remaining semantic R&D path is now a durable learned-policy artifact with
explicit no-regress constraints and holdout proof, plus separate VeriSchema
query/document-construction work.
