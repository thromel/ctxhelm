...
artifacts after Phase 280. This audit is not a release claim by itself; it
...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   runtime/default completion. Promotion still needs a separate cross-repo or
   cross-slice holdout that applies the exported policy outside the training
   snapshot.
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   metadata. The next semantic step is holdout evaluation of exported policy
   profiles. Increasing the local-fastembed document cap, adding broad Python
   path metadata, and adding another hand-written route are already rejected
   paths.
...
with support, staleness, and holdout status. The remaining semantic R&D path is
now holdout evaluation of exported learned-policy profiles with explicit
no-regress constraints, plus separate VeriSchema query/document-construction
work.
