...
artifacts after Phase 277. This audit is not a release claim by itself; it
...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   VeriSchema still regress. The remaining semantic path should not add another
   handwritten route without explicit no-regress proof.
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   displacement families that any next experiment must guard against. The next
   semantic step is a learned/listwise experiment with explicit no-regress
   constraints or corpus/profile-specific policy. Increasing the local-fastembed
...
the displacement pressure behind those losses, and Phase 277 rejects the
path-family budget rule as a general solution. The remaining semantic R&D path
is now learned/listwise routing with explicit no-regress constraints, plus
