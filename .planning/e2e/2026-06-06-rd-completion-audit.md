...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   policy-enabled reranking the same protected-source floor. The right next work
   is task-conditioned query construction, alternate local model/fusion
   evaluation, or query-family reranker routing with churn constraints, not
   unconditional semantic or reranker promotion.
