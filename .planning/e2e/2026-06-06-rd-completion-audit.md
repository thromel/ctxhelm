...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   and surrounded by mixed/noise holds, so the next step is a measured eval-only
   support-profile-routed variant.
...
4. Run an eval-only support-profile-routed semantic experiment before any
   runtime semantic routing is added. It should include route-candidate support
   profiles, exclude profile noise holds, and compare recall, target churn, and
   named regressions against default.
...
the remaining semantic path to eval-only support-profile routing, still not
runtime promotion. Phase 253 reduces memory risk by proving cross-repo Codex
