...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   `dependency_co_change` support. The next semantic work should treat
   corrected Jina as a diagnostic backend and focus on fusion/top-K ordering
   for supported semantic candidates or a much narrower learned separator, not
   more single-query expansion, MiniLM model swaps, next-read promotion,
   document expansion before supported fusion is resolved, or default model
   promotion.
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   `schema_agent/core/state.py` fusion/top-K ordering gap. The next semantic
   step is a fusion/selection constraint for supported semantic candidates, or
   a materially narrower learned separator, not runtime promotion. Increasing the
...
`schema_agent/core/state.py` with `dependency_co_change` support. The remaining
semantic R&D path is fusion/selection constraints over supported improved
candidates or a much narrower learned separator, not more same-range doubling,
not coarse profile-key
