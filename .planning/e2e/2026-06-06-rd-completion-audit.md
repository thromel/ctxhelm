...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   no safe semantic-corroborated fusion, and a `6.01x` runtime ratio. The next
   semantic work should treat corrected Jina as a diagnostic backend and focus
   on fusion/selection constraints or a much narrower learned separator, not
   more single-query expansion or default model promotion.
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   candidate-sibling path aliases, generic path-concept query terms, and Jina
   as the default local-fastembed model are already rejected paths. Corrected
   Jina remains useful diagnostic evidence for candidate quality.
...
remains too slow for default promotion. The remaining semantic R&D path is
...
tail-slot reranking, or Jina as runtime/default policy, and not runtime/default
promotion.
