...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   tail-slot oracle can recover that target without churn. The next semantic
   work should treat corrected Jina as a diagnostic backend and replace the
   oracle with a source-free predictor for supported semantic candidates, or a
   much narrower learned separator, not more single-query expansion, MiniLM
   model swaps, next-read promotion, document expansion before supported fusion
   is resolved, or default model promotion.
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   gold-backed tail-slot oracle can recover that target without churn. The next
   semantic step is a source-free predictor for supported semantic candidates,
   or a materially narrower learned separator, not runtime promotion.
...
that target without churn. The remaining semantic R&D path is source-free
prediction for supported improved candidates or a much narrower learned
separator, not more same-range doubling, not coarse profile-key
