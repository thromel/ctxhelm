...
1. Semantic is not yet a proven default-lift channel. It is useful and bounded,
   ...
   dropped targets but also 232 retained non-targets. The next semantic work
   should treat corrected Jina as a diagnostic backend and evaluate a held-out
   learned separator over these retention cells, not more single-query
   expansion, MiniLM model swaps, next-read promotion, broad document expansion,
   supported-shape promotion, or default model promotion.
...
4. Move semantic R&D from routing existing clean-looking support profiles to a
   ...
   view and exposes 12 recoverable dropped-target family rows. The next
   semantic step is a materially narrower learned separator over retained and
   dropped cells, not runtime promotion.
...
study, but not enough for a default policy. The remaining semantic R&D path is
a held-out learned separator over candidate-retention cells, not more
same-range doubling, not coarse profile-key relaxation, not generic role words,
not current cross-repo aggregation, not candidate-path hints, candidate-sibling
