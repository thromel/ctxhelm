...
The full JSON report is intentionally kept under ignored `.ctxpack/e2e/`.

## Suite

- Manifest version: `ctxpack-benchmark-corpus-v2.5`
...
| ctxpack | 10 | 0.2270 | 0.2742 | -0.0472 | 0.2618 | 0.0000 | 7.7s |
...
- `source` / `ranked_below_budget_dependency` / `crates/ctxpack-compiler/src/*.rs` / parserPrecision
- `unknown` / `no_candidate_signal` / `scripts/*.sh` / storage
- `docs` / `no_candidate_signal` / `.planning/*.md` / storage

## Interpretation

The multi-repo benchmark path works and remains source-free. The evidence is also clear: ctxpack should not claim quality lift yet. RefactoringMiner is near lexical parity, while ctxpack trails lexical more clearly. Phase 62 should evaluate production local embeddings on the same corpus; Phase 64 should target the repeated docs/scripts/planning and compiler-source gap families.
