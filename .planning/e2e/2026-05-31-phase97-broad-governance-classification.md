...
historical ctxpack commits used natural task wording such as "evaluate
...
```bash
cargo test -p ctxpack-compiler governance_tasks_add_root_planning_docs_as_candidates -- --nocapture
...
cargo run --release -p ctxpack -- eval proof \
  ...
  --format json > .ctxpack/e2e/phase97-broad-governance-classification-proof.json

python3 scripts/check-product-proof.py \
  .ctxpack/e2e/phase97-broad-governance-classification-proof.json
```
...
- ctxpack File Recall@10 improves from `0.44603175` to `0.47460318`
- ctxpack Source Recall@10 improves from `0.6333333` to `0.7166667`
- ctxpack broad context-area recall improves from `0.0` to `1.0`
- ctxpack true source `no_candidate_signal` gaps are converted into
