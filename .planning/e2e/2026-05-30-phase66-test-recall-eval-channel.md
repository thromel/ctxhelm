...
```bash
cargo test -p ctxpack-compiler context_ranking_keeps_validation_tests_inside_budget -- --nocapture
cargo test -p ctxpack-index related_tests_prefers_exact_tests_for_higher_ranked_source_seeds -- --nocapture
cargo run -p ctxpack -- \
...
produced `.ctxpack/e2e/phase66-test-recall-proof.json`.
...
| ctxpack | `ctxpack_default` | `match` | 0.1879 | 0.1984 | -0.0105 | 1.0000 | 0.1700 |

## Decision

Phase 66 improves the product proof by correctly measuring validation-test recommendations. It does not make ctxpack production-ready for default promotion. The default still trails lexical on RefactoringMiner and does not beat lexical on ctxpack.

## Follow-Up

- Parser/precision work for repeated source dependency misses.
- Storage/indexing coverage for `.planning`, `docs`, and `scripts` gaps in the ctxpack corpus.
- Protected evidence budget improvements for ctxpack.
