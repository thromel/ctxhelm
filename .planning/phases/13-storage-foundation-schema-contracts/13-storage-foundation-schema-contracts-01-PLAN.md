...
- D-01: Default durable storage remains under `CTXPACK_HOME` or `~/.ctxpack`.
...
<action>Add workspace dependency `rusqlite = { version = "0.39.0", features = ["bundled"] }` in root `Cargo.toml`, then add `rusqlite = { workspace = true }` to `crates/ctxpack-index/Cargo.toml`. Do not add SQLite dependencies to `crates/ctxpack`, `ctxpack-core`, `ctxpack-compiler`, or `ctxpack-mcp` in this plan.</action>
<acceptance_criteria>
- Root `Cargo.toml` contains workspace dependency key `rusqlite`.
- `crates/ctxpack-index/Cargo.toml` contains `rusqlite = { workspace = true }`.
...
<action>Create `crates/ctxpack-index/src/storage.rs` with `STORAGE_SCHEMA_VERSION: u32 = 1`, `StoreConfig`, `StorePaths`, `StorageMetadata`, `StoragePrivacyMode`, `StorageOpenMode`, and `StorageReport`. Implement default path resolution that uses `CTXPACK_HOME/repos/<repo-id>/ctxpack.sqlite3`, then `HOME/.ctxpack/repos/<repo-id>/ctxpack.sqlite3`, then `.ctxpack/repos/<repo-id>/ctxpack.sqlite3`. Include explicit override support through `StoreConfig { path_override: Option<PathBuf> }`.</action>
<acceptance_criteria>
- `crates/ctxpack-index/src/storage.rs` defines `STORAGE_SCHEMA_VERSION`.
- `StorePaths` exposes the concrete SQLite path as `database_path`.
- Default path tests assert the path ends with `repos/<repo-id>/ctxpack.sqlite3`.
...
- Initialization does not create or modify `.ctxpack/` inside the target repo unless an explicit override points there.
</acceptance_criteria>
</task>
...
<acceptance_criteria>
- `ctxpack_index::initialize_store` compiles from an external crate test or existing integration test.
