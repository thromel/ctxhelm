...
- **Scope:** Out of scope for Plan 02; the failing file is `crates/ctxpack/tests/cli_compat.rs`, which Plan 02 must not edit because Plan 01 may own it in parallel.
- **Failure:** `mcp_protocol_uses_explicit_repo_from_wrong_cwd` returned `pack resource is not available in this MCP session; call prepare_task first: ctxpack://pack/brief.json`.
...
- **Scope:** Out of scope for Plan 02; diffs are in `crates/ctxpack/tests/cli_compat.rs` and `crates/ctxpack-mcp/src/lib.rs`, while Plan 02 owns only `crates/ctxpack-core/src/init.rs`.
