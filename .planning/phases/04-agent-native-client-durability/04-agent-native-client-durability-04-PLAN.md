...
must_haves:
  truths:
    ...
    - "A required mode exists so maintainers can fail CI/local validation if an installed real client cannot exercise ctxpack."
  artifacts:
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 4: Both scripts support a required mode variable such as `CTXPACK_REQUIRE_REAL_CLIENT=1` and a default optional mode that can skip unavailable clients with a zero exit.
  </behavior>
  <action>Add compatibility tests in `crates/ctxpack/tests/cli_compat.rs` that read the script files and run `bash -n` on them. Create initial script files if needed so the tests can go green in Task 2. The tests must guard contract text and skip/required semantics, not depend on Codex or Claude authentication.</action>
  <verify>
    <automated>cargo test -p ctxpack --test cli_compat real_client_smoke_scripts -- --nocapture</automated>
  </verify>
...
<task type="auto" tdd="true">
  ...
  <behavior>
    - Test 1: Each wrapper always runs `scripts/smoke-mcp-protocol.sh` first using `CTXPACK_SMOKE_REPO`, `CTXPACK_SMOKE_TASK`, and an isolated `CTXPACK_HOME`.
    - Test 2: If the client binary is absent and `CTXPACK_REQUIRE_REAL_CLIENT` is unset, the wrapper prints a clear skip message and exits 0.
    - Test 3: If the client binary is absent or cannot produce contract evidence and `CTXPACK_REQUIRE_REAL_CLIENT=1`, the wrapper exits non-zero.
    ...
    - Test 5: Default verification attempts installed clients unless `CTXPACK_SKIP_REAL_CLIENT=1`; required mode with `CTXPACK_REQUIRE_REAL_CLIENT=1` fails if an installed/authenticated client cannot produce the tool-call evidence.
  </behavior>
  <action>Implement `scripts/smoke-codex-mcp.sh` and `scripts/smoke-claude-mcp.sh` per D-01 and D-02. For Codex, isolate `CODEX_HOME` in a temp dir, use `codex exec --ephemeral --ignore-user-config --skip-git-repo-check --cd "$outside_cwd" --json --output-last-message "$out"` with `-c` MCP server config overrides or a temp config that launches the local ctxpack binary/manifest path. For Claude, create a temp MCP config JSON and run `claude -p --bare --no-session-persistence --strict-mcp-config --mcp-config "$config" --allowedTools "mcp__ctxpack__prepare_task,mcp__ctxpack__get_pack" --permission-mode bypassPermissions --output-format stream-json` when available. Both prompts must require explicit `repo="$CTXPACK_SMOKE_REPO"` in prepare_task/get_pack. The scripts must validate machine-checkable tool-call evidence from JSON event/tool transcripts or explicit server-side instrumentation that records tool name plus repo path for both `prepare_task` and `get_pack`; a final assistant marker or last-message text alone is not sufficient. Treat auth/model/client refusal as a clear optional skip unless `CTXPACK_REQUIRE_REAL_CLIENT=1`. Default verification should attempt installed clients; `CTXPACK_SKIP_REAL_CLIENT=1` is allowed only for syntax/contract tests and non-authenticated environments. Do not permanently edit user Codex/Claude config.</action>
  <verify>
    <automated>bash -n scripts/smoke-codex-mcp.sh && bash -n scripts/smoke-claude-mcp.sh && CTXPACK_SKIP_REAL_CLIENT=1 bash scripts/smoke-codex-mcp.sh && CTXPACK_SKIP_REAL_CLIENT=1 bash scripts/smoke-claude-mcp.sh && cargo test -p ctxpack --test cli_compat real_client_smoke_scripts -- --nocapture</automated>
  </verify>
...
- Default installed-client attempt: `CTXPACK_SMOKE_REPO="$PWD" bash scripts/smoke-codex-mcp.sh`
- Default installed-client attempt: `CTXPACK_SMOKE_REPO="$PWD" bash scripts/smoke-claude-mcp.sh`
- Required when installed/authenticated: `CTXPACK_REQUIRE_REAL_CLIENT=1 CTXPACK_SMOKE_REPO="$PWD" bash scripts/smoke-codex-mcp.sh`
- Required when installed/authenticated: `CTXPACK_REQUIRE_REAL_CLIENT=1 CTXPACK_SMOKE_REPO="$PWD" bash scripts/smoke-claude-mcp.sh`
