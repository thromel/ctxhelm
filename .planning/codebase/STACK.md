...
- Rust 2021 edition - all executable and library code lives in the Cargo workspace rooted at `Cargo.toml`.
  - CLI binary: `crates/ctxpack/src/main.rs`
  - Shared contracts and init logic: `crates/ctxpack-core/src/lib.rs`, `crates/ctxpack-core/src/contracts.rs`, `crates/ctxpack-core/src/init.rs`, `crates/ctxpack-core/src/privacy.rs`, `crates/ctxpack-core/src/repo.rs`
  - Repository inventory, search, symbols, git history, and dependency graph logic: `crates/ctxpack-index/src/lib.rs`
  - Context plan, pack, cards, and eval compilation: `crates/ctxpack-compiler/src/lib.rs`
  - Stdio MCP runtime: `crates/ctxpack-mcp/src/lib.rs`
...
- JSON and TOML - generated configuration and protocol payload formats.
  - Cargo manifests: `Cargo.toml`, `crates/ctxpack/Cargo.toml`, `crates/ctxpack-core/Cargo.toml`, `crates/ctxpack-index/Cargo.toml`, `crates/ctxpack-compiler/Cargo.toml`, `crates/ctxpack-mcp/Cargo.toml`
  - Generated repo config template: `.ctxpack/ctxpack.toml` from `crates/ctxpack-core/src/init.rs`
  - Generated Claude MCP snippet: `.ctxpack/adapters/claude-mcp.json` from `crates/ctxpack-core/src/init.rs`
  - Generated OpenCode snippet: `.ctxpack/adapters/opencode.jsonc.snippet` from `crates/ctxpack-core/src/init.rs`
...
- Runtime entrypoints are local command-line processes, not web servers.
  - `ctxpack` CLI command dispatch is in `crates/ctxpack/src/main.rs`.
  - `ctxpack serve-mcp` starts a JSON-RPC-over-stdio MCP server via `ctxpack_mcp::run_stdio_server()` in `crates/ctxpack/src/main.rs` and `crates/ctxpack-mcp/src/lib.rs`.
...
- External process tools used by the implementation:
  - `git` is invoked with `std::process::Command` for co-change history, current diff, historical commit samples, and historical evals in `crates/ctxpack-index/src/lib.rs` and `crates/ctxpack-compiler/src/lib.rs`.
  - `tar` is invoked when extracting historical revision snapshots in `crates/ctxpack-compiler/src/lib.rs`.
...
- Primary runtime configuration is CLI arguments and repo-local files.
  - CLI argument parsing is centralized in `crates/ctxpack/src/main.rs`.
  - Repository discovery requires a `.git` ancestor and is implemented in `crates/ctxpack-core/src/repo.rs`.
- `CTXPACK_HOME` controls private local state location in `crates/ctxpack-index/src/lib.rs`.
  - If `CTXPACK_HOME` is absent, `HOME/.ctxpack` is used.
  - If neither is available, `.ctxpack` relative to the current process is used.
...
- `git` CLI available on `PATH`.
  - Required for `co-changes`, `current_diff`, historical commit samples, and historical retrieval evals in `crates/ctxpack-index/src/lib.rs` and `crates/ctxpack-compiler/src/lib.rs`.
- `tar` CLI available on `PATH`.
  - Required for historical eval snapshot extraction in `crates/ctxpack-compiler/src/lib.rs`.
