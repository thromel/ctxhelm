...
- Issue: `load_or_build_inventory` loads `~/.ctxpack/repos/<repo-id>/inventory.json` whenever it exists and only rebuilds when the file is missing. It does not compare current filesystem state, stored hashes, ignore files, or option changes before using cached entries.
- Files: `crates/ctxpack-index/src/lib.rs`
- Impact: `lexical_search`, `symbol_search`, `related_tests`, `test_map`, `dependency_edges`, `prepare_context_plan`, MCP `search`, MCP `prepare_task`, and MCP `get_pack` can miss newly created files, include deleted paths, or use stale file roles until `ctxpack index` refreshes the cache.
...
- Files: `crates/ctxpack-index/src/lib.rs`, `crates/ctxpack-compiler/src/lib.rs`, `crates/ctxpack-mcp/src/lib.rs`
...
- Files: `crates/ctxpack-index/src/lib.rs`
- Impact: `related_tests`, `test_map`, MCP `ctxpack://repo/test-map`, and context plans can recommend commands that do not run in npm/yarn/bun projects or workspaces with custom runners.
...
- Issue: `prepare-task`, `get-pack`, MCP `prepare_task`, and MCP `get_pack` append traces under `~/.ctxpack/repos/<repo-id>/traces.jsonl`. Search-like calls also call `load_or_build_inventory`, which may write an inventory under `~/.ctxpack/repos/<repo-id>/inventory.json`.
- Files: `crates/ctxpack/src/main.rs`, `crates/ctxpack-mcp/src/lib.rs`, `crates/ctxpack-index/src/lib.rs`
...
- Files: `crates/ctxpack-index/src/lib.rs`
- Trigger: Run any API that creates an inventory, change repo files or ignore rules, then call `ctxpack search`, `ctxpack symbols`, `ctxpack prepare-task`, MCP `search`, or MCP `prepare_task` without `ctxpack index`.
- Workaround: Run `cargo run -p ctxpack -- index --repo /path/to/repo` after file layout or ignore-policy changes.

**MCP pack resources are process-local and unavailable across sessions:**
- Symptoms: `prepare_task` returns `ctxpack://pack/<task-id>/<budget>` URIs, but `resources/read` for those URIs only succeeds in the same MCP server process because the cache is an in-memory `OnceLock<Mutex<BTreeMap<...>>>`.
- Files: `crates/ctxpack-mcp/src/lib.rs`
- Trigger: A client reconnects, starts another server process, or sends `resources/read` to a different ctxpack process than the one that handled `prepare_task`.
...
- Files: `crates/ctxpack-index/src/lib.rs`, `crates/ctxpack-mcp/src/lib.rs`, `crates/ctxpack-compiler/src/lib.rs`
...
- Risk: MCP `get_pack`, `ctxpack get-pack`, and session-scoped pack resources include target and test snippets from safe files. This is useful for local agents, but it means the privacy guarantee depends entirely on correct inventory filtering.
- Files: `crates/ctxpack-compiler/src/lib.rs`, `crates/ctxpack-mcp/src/lib.rs`
...
- Files: `crates/ctxpack-index/src/lib.rs`, `crates/ctxpack-compiler/src/lib.rs`
...
- Files: `crates/ctxpack-index/src/lib.rs`, `crates/ctxpack-mcp/src/lib.rs`
...
- Files: `crates/ctxpack-mcp/src/lib.rs`, `crates/ctxpack-core/src/contracts.rs`
...
- Files: `crates/ctxpack-compiler/src/lib.rs`, `crates/ctxpack-index/src/lib.rs`
...
- What's not tested: Full binary-level command execution for `ctxpack index`, `search`, `prepare-task`, `get-pack`, `serve-mcp`, `cards generate`, and eval subcommands.
- Files: `crates/ctxpack/src/main.rs`
