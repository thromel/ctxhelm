...
files_modified:
  ...
  - crates/ctxpack-compiler/Cargo.toml
  - crates/ctxpack-mcp/Cargo.toml
...
must_haves:
  truths:
    - "User can run an installed or built `ctxpack --version` and see the v1.1 release version."
  ...
  artifacts:
    ...
    - path: "crates/ctxpack/src/main.rs"
      provides: "Clap package version wiring for `ctxpack --version`"
    - path: "LICENSE"
...
<objective>
Establish release identity for ctxpack v1.1.
...
<interfaces>
Current CLI shape in `crates/ctxpack/src/main.rs`:
```rust
#[derive(Debug, Parser)]
...
<task type="auto" tdd="true">
  ...
  <files>Cargo.toml, crates/ctxpack/Cargo.toml, crates/ctxpack-core/Cargo.toml, crates/ctxpack-index/Cargo.toml, crates/ctxpack-compiler/Cargo.toml, crates/ctxpack-mcp/Cargo.toml, LICENSE</files>
  <behavior>
    - Test 1: `cargo metadata --no-deps --format-version 1` reports `1.1.0` for `ctxpack`, `ctxpack-core`, `ctxpack-index`, `ctxpack-compiler`, and `ctxpack-mcp`.
  ...
  <action>Update package metadata per D-02 and PKG-03. Set the v1.1 release identity to `1.1.0` across all workspace packages using workspace inheritance where Cargo accepts it and explicit per-crate fields where nested readme paths require it. Add repository URL metadata for the public ctxpack repository, a release-appropriate description, and an honest `rust-version` based on the supported local baseline from research. Create a root `LICENSE` file for MIT. Do not publish to crates.io, add package-manager config, or change dependency versions unless Cargo metadata requires a syntax correction.</action>
  <verify>
    <automated>cargo metadata --no-deps --format-version 1 >/tmp/ctxpack-metadata.json && python3 - <<'PY'
import json
from pathlib import Path
data=json.loads(Path('/tmp/ctxpack-metadata.json').read_text())
pkgs={p['name']:p for p in data['packages'] if p['name'].startswith('ctxpack')}
expected={'ctxpack','ctxpack-core','ctxpack-index','ctxpack-compiler','ctxpack-mcp'}
...
PY</automated>
  ...
  <done>All ctxpack workspace packages carry consistent v1.1 release metadata and a root MIT license exists.</done>
</task>
...
<task type="auto" tdd="true">
  ...
  <behavior>
    - Test 1: `ctxpack --version` exits 0 and prints `ctxpack 1.1.0`.
    - Test 2: `ctxpack --help` still lists existing core commands including `init`, `prepare-task`, `get-pack`, `eval`, and `serve-mcp`.
    - Test 3: `ctxpack prepare-task --help` and MCP-facing command behavior remain unchanged except for top-level version support.
  </behavior>
  <action>Add Clap version wiring to the top-level `Cli` command so `ctxpack --version` uses the package version from `crates/ctxpack/Cargo.toml`. Extend `crates/ctxpack/tests/cli_compat.rs` with a focused binary test for `--version` and keep the existing help/core-command assertions intact. Do not alter subcommand names, JSON output fields, MCP tool names, or retrieval behavior.</action>
  <verify>
    <automated>cargo test -p ctxpack --test cli_compat version -- --nocapture && cargo run -p ctxpack -- --version && cargo run -p ctxpack -- --help</automated>
  </verify>
  <done>`ctxpack --version` is a working support diagnostic and help output remains compatible.</done>
</task>
...
- `cargo test -p ctxpack --test cli_compat version -- --nocapture`
...
<success_criteria>
Plan 01 is complete when ctxpack has a consistent v1.1 identity across Cargo metadata, license files, and binary diagnostics without changing runtime context-selection behavior.
</success_criteria>
