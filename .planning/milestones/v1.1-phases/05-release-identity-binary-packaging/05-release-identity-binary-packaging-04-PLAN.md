...
must_haves:
  truths:
    - "User can read the normal install path for a prebuilt v1.1 archive, verify checksums, install the binary on PATH, and run `ctxpack --version` plus `ctxpack --help` without a source checkout."
...
<task type="auto" tdd="true">
  ...
  <behavior>
    - Test 1: `scripts/check-release-docs.sh` exists, passes `bash -n`, and exits non-zero if README or `docs/release.md` omit `ctxpack --version`, `ctxpack --help`, `v1.1.0`, checksum verification, or `cargo install --git ... --tag v1.1.0 ... --locked`.
...
<task type="auto" tdd="true">
  ...
  <files>README.md, docs/release.md, scripts/check-release-docs.sh, crates/ctxpack/tests/release_packaging.rs</files>
  <behavior>
    - Test 1: README has a short normal-user path: download archive, verify SHA-256 checksum, extract/move `ctxpack` onto PATH, run `ctxpack --version`, and run `ctxpack --help`.
    ...
    - Test 3: Documentation includes fallback commands `cargo install --git <repo-url> --tag v1.1.0 ctxpack --locked`, `cargo install --path crates/ctxpack --locked`, and `cargo build -p ctxpack --release --locked`.
  ...
  <verify>
    <automated>bash scripts/check-release-docs.sh && cargo test -p ctxpack --test release_packaging release_docs -- --nocapture && cargo run -p ctxpack -- --version && cargo run -p ctxpack -- --help</automated>
  </verify>
