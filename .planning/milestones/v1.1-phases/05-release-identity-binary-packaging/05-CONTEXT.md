...
### Version Identity
- Add or verify `ctxhelm --version` so support/debug starts with a simple installed-binary diagnostic.
...
- Audit archive contents for `.ctxhelm`, traces, request logs, temp homes, secrets, absolute local paths, target debris, and unintended caches.
- Keep stdout cleanliness for `ctxhelm serve-mcp`; packaging scripts must not introduce stdout logging into the MCP server.
...
### Reusable Assets
- `crates/ctxhelm/src/main.rs` owns the Clap CLI and already exposes the user-facing command tree.
...
- Keep ctxhelm local-first, read-only, and agent-native.
...
### Integration Points
- CLI version/help output belongs in `crates/ctxhelm/src/main.rs` and package metadata.
...
- Add tests around `ctxhelm --version` and package metadata where practical.
