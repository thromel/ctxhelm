...
must_haves:
  ...
  key_links:
    - from: "crates/ctxhelm-mcp/src/tools.rs"
      to: "crates/ctxhelm-mcp/src/resources.rs"
...
<task type="auto" tdd="true">
  ...
  <files>crates/ctxhelm-mcp/src/resources.rs, crates/ctxhelm-mcp/src/lib.rs, crates/ctxhelm/tests/cli_compat.rs</files>
  <behavior>
    - Test 1: `ctxhelm://pack/guide` says pack resources are MCP-session scoped and `get_pack` is the durable reconnect-safe way to materialize a pack.
    - Test 2: Missing pack-resource errors include `session-scoped`, `same MCP server process`, and `call prepare_task first`.
    - Test 3: A subprocess test starts `ctxhelm serve-mcp`, calls `prepare_task` with explicit repo and captures a pack URI, then starts a second server process and verifies `resources/read` for that URI returns the clear session-scoped diagnostic.
  </behavior>
  <action>Preserve session-scoped resources for AGNT-02 rather than adding reconstruction. Update `pack_guide_markdown` and `read_pack_resource` diagnostics to explain that pack URIs only work in the same MCP server process and that clients should call `get_pack` again after reconnect/restart. Add in-process tests in `crates/ctxhelm-mcp/src/lib.rs` and a binary subprocess restart test in `crates/ctxhelm/tests/cli_compat.rs`. The subprocess test should use the same fixture style as Plan 01 and must pass explicit `repo` to `prepare_task`; resource reads do not need a repo argument because the diagnostic is about pack cache session scope.</action>
  <verify>
    <automated>cargo test -p ctxhelm-mcp pack_resource -- --nocapture && cargo test -p ctxhelm --test cli_compat pack_resource -- --nocapture</automated>
  </verify>
...
<task type="auto" tdd="true">
  ...
  <files>crates/ctxhelm-mcp/src/resources.rs, crates/ctxhelm-mcp/src/lib.rs</files>
  <behavior>
  ...
  <verify>
    <automated>cargo test -p ctxhelm-mcp pack_resource_cache -- --nocapture && cargo test --workspace</automated>
  </verify>
