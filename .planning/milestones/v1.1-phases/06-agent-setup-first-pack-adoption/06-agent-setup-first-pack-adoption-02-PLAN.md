...
Purpose: Users should be able to wire ctxhelm into Codex, Claude Code, Cursor, and OpenCode without hidden global mutations or stale static context.
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 3: Codex guidance contains a copy/paste setup command such as `codex mcp add ctxhelm -- ctxhelm serve-mcp` or an equivalent config snippet, and explicitly says ctxhelm does not apply it automatically.
    - Test 4: Claude guidance stays project-local or mergeable and `.ctxhelm/adapters/claude-mcp.json` remains valid JSON.
...
<task type="auto" tdd="true">
  ...
  <action>Refresh `AGENTS_SECTION`, `CURSOR_RULE`, `CLAUDE_BUGFIX_COMMAND`, `OPENCODE_SNIPPET`, and `CODEX_MCP_SETUP` to satisfy ADPT-02 and ADPT-03. Keep snippets small and dynamic; do not embed repo file lists, generated cards, source examples, or large context. Add absolute-binary troubleshooting guidance only as concise text such as "if the agent cannot spawn `ctxhelm`, replace it with the absolute path from `which ctxhelm`"; do not attempt to resolve a user-specific path in generated repo files.</action>
  <verify>
    <automated>cargo test -p ctxhelm-core adapter -- --nocapture && cargo test -p ctxhelm-core init -- --nocapture</automated>
  </verify>
...
<verification>
- `cargo test -p ctxhelm-core adapter -- --nocapture`
