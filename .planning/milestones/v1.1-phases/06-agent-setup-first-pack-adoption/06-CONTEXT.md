...
This phase turns the v1.1 binary into a usable first-run agent setup experience. It covers `ctxhelm init` reporting, generated Codex/Claude/Cursor/OpenCode setup guidance, setup validation, and a first useful prepare-task/get-pack journey. It does not publish a package, alter retrieval ranking, auto-edit global agent configuration, or replace the agent clients' native permission and editing flows.
...
- `ctxhelm init` should clearly report repo-local files written, skipped, and unchanged.
...
- Init should continue to write only repo-local ctxhelm/adaptor artifacts unless a future explicit apply mode is designed separately.
...
- Cursor and OpenCode artifacts should remain thin rules/snippets that direct the agent to dynamic ctxhelm MCP calls.
...
- Use deterministic MCP protocol smoke as the hard proof of ctxhelm behavior before asking users to debug agent auth/model issues.
...
- The first-pack journey should work from a real repo using installed `ctxhelm`, not only `cargo run`.

### the agent's Discretion
- The planner may choose whether setup validation is a new `ctxhelm` subcommand, a script, or an extension of existing init output, provided it is testable and user-facing.
...
### Reusable Assets
- `crates/ctxhelm-core/src/init.rs` owns generated `AGENTS.md`, `.ctxhelm/ctxhelm.toml`, Cursor rule, Claude command, Claude MCP snippet, and OpenCode snippet templates.
- `crates/ctxhelm/src/main.rs` owns `init`, `prepare-task`, `get-pack`, `serve-mcp`, and CLI rendering.
- `crates/ctxhelm/tests/cli_compat.rs` already guards init/adaptor guidance and MCP compatibility.
...
- Keep adapter text concise and dynamic; static files should tell agents to call ctxhelm, not embed repo context.
...
### Integration Points
- Init reports and adapter templates connect through `ctxhelm-core::init`.
...
- Consider adding `ctxhelm doctor` or `ctxhelm setup-check` if a dedicated command makes validation simpler.
