...
must_haves:
  truths:
    - "Maintainer can split ctxhelm-index internals while crate-root public APIs remain import-compatible."
  ...
  key_links:
    - from: "crates/ctxhelm-index/src/lib.rs"
...
<objective>
Split `ctxhelm-index` into focused private modules behind the existing crate-root facade.
...
<task type="auto" tdd="true">
  ...
  <files>crates/ctxhelm-index/src/lib.rs, crates/ctxhelm-index/src/inventory.rs, crates/ctxhelm-index/src/search.rs</files>
  <read_first>
    - crates/ctxhelm-index/src/lib.rs
  ...
  <behavior>
    - `ctxhelm_index::build_inventory`, `write_inventory`, `load_inventory`, `load_or_build_inventory`, `inventory_path`, `InventoryOptions`, `RepoInventory`, `InventoryReport`, `FileInventoryEntry`, `SearchOptions`, `SearchResult`, and `lexical_search` remain import-compatible.
    - Existing inventory and lexical search tests still pass.
  </behavior>
  <action>
    Move inventory types/functions/path classification helpers from `crates/ctxhelm-index/src/lib.rs` into `crates/ctxhelm-index/src/inventory.rs`. Move lexical search types/functions/query scoring helpers into `crates/ctxhelm-index/src/search.rs`. Keep `is_sensitive_path`, `is_generated_path`, `classify_path`, and `language_for_path` available to sibling modules with `pub(crate)` only where needed. Replace the moved sections in `lib.rs` with `mod inventory; mod search;` and `pub use inventory::{...}; pub use search::{...};` for the exact public API names listed in this plan. Do not change scoring constants, generated/sensitive classification behavior, inventory JSON fields, or search result shape.
  </action>
  <verify>
    <automated>cargo test -p ctxhelm-index inventory -- --nocapture</automated>
    <automated>cargo test -p ctxhelm-index lexical_search -- --nocapture</automated>
  </verify>
  <acceptance_criteria>
    - `test -f crates/ctxhelm-index/src/inventory.rs && test -f crates/ctxhelm-index/src/search.rs` succeeds.
    - `rg -n '^mod inventory;|^mod search;|pub use inventory::|pub use search::' crates/ctxhelm-index/src/lib.rs` finds facade declarations.
    - `rg -n 'pub fn build_inventory|pub fn write_inventory|pub fn load_or_build_inventory|pub fn lexical_search|pub struct SearchOptions|pub struct SearchResult' crates/ctxhelm-index/src/inventory.rs crates/ctxhelm-index/src/search.rs` finds the moved public items.
    - `cargo test -p ctxhelm-index inventory -- --nocapture` passes.
    - `cargo test -p ctxhelm-index lexical_search -- --nocapture` passes.
  </acceptance_criteria>
...
<task type="auto" tdd="true">
  ...
  <files>crates/ctxhelm-index/src/lib.rs, crates/ctxhelm-index/src/symbols.rs, crates/ctxhelm-index/src/related_tests.rs, crates/ctxhelm-index/src/dependencies.rs</files>
  <read_first>
    - crates/ctxhelm-index/src/lib.rs
    - crates/ctxhelm-index/src/inventory.rs
    - crates/ctxhelm-index/src/search.rs
  ...
  <behavior>
    - Symbol, related-test, and dependency APIs remain exported from `ctxhelm_index`.
    - Existing tests for symbol extraction, related tests, test map, dependency edges, and related dependency edges still pass.
  </behavior>
  <action>
    Move symbol extraction/search into `crates/ctxhelm-index/src/symbols.rs`, test mapping and command inference into `crates/ctxhelm-index/src/related_tests.rs`, and dependency/import graph logic into `crates/ctxhelm-index/src/dependencies.rs`. Add `mod symbols; mod related_tests; mod dependencies;` and exact `pub use` exports in `lib.rs`. Use shared `inventory` helpers instead of duplicating path classification. Preserve struct derives, serde rename rules, package-manager command strings, dependency edge `kind` values, symbol kind strings, and result sorting.
  </action>
  <verify>
  ...
  <acceptance_criteria>
    - `test -f crates/ctxhelm-index/src/symbols.rs && test -f crates/ctxhelm-index/src/related_tests.rs && test -f crates/ctxhelm-index/src/dependencies.rs` succeeds.
    - `rg -n '^mod symbols;|^mod related_tests;|^mod dependencies;|pub use symbols::|pub use related_tests::|pub use dependencies::' crates/ctxhelm-index/src/lib.rs` finds facade declarations.
    - `rg -n 'pub fn symbol_search|pub struct CodeSymbol|pub fn related_tests|pub fn test_map|pub fn dependency_edges|pub fn related_dependency_edges' crates/ctxhelm-index/src/symbols.rs crates/ctxhelm-index/src/related_tests.rs crates/ctxhelm-index/src/dependencies.rs` finds moved APIs.
    - All three focused `cargo test -p ctxhelm-index ...` commands pass.
  </acceptance_criteria>
...
<task type="auto" tdd="true">
  ...
  <files>crates/ctxhelm-index/src/lib.rs, crates/ctxhelm-index/src/git.rs, crates/ctxhelm-index/src/traces.rs</files>
  <read_first>
    - crates/ctxhelm-index/src/lib.rs
    - crates/ctxhelm-index/src/inventory.rs
    - crates/ctxhelm-index/src/dependencies.rs
  ...
  <verify>
    ...
    <automated>cargo test -p ctxhelm-index eval_traces -- --nocapture</automated>
    <automated>cargo test -p ctxhelm --test cli_compat</automated>
  </verify>
  <acceptance_criteria>
    - `test -f crates/ctxhelm-index/src/git.rs && test -f crates/ctxhelm-index/src/traces.rs` succeeds.
    - `rg -n '^mod git;|^mod traces;|pub use git::|pub use traces::' crates/ctxhelm-index/src/lib.rs` finds facade declarations.
    - `rg -n 'pub fn co_change_hints|pub fn current_diff_summary|pub fn historical_commit_samples|pub fn append_eval_trace|pub fn list_eval_traces' crates/ctxhelm-index/src/git.rs crates/ctxhelm-index/src/traces.rs` finds moved APIs.
    - `cargo test -p ctxhelm-index` passes.
    - `cargo test -p ctxhelm --test cli_compat` passes.
  </acceptance_criteria>
  <done>`ctxhelm-index` is split by concern with stable crate-root exports and passing compatibility guardrails.</done>
</task>

</tasks>

<verification>
Run `cargo test -p ctxhelm-index`, `cargo test -p ctxhelm-compiler`, `cargo test -p ctxhelm-mcp`, and `cargo test -p ctxhelm --test cli_compat`.
</verification>
