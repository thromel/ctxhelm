...
tags: [rust, module-boundaries, ctxhelm-compiler, ctxhelm-mcp, compatibility]

# Dependency graph
requires:
  - phase: 01-compatibility-guardrails-module-boundaries
    provides: "CLI, JSON contract, MCP, and ctxhelm-index compatibility guardrails from Plans 01-03"
provides:
  - "ctxhelm-compiler split into planning, packs, cards, and eval modules behind stable crate-root exports"
  - "ctxhelm-mcp split into protocol, schemas, tools, resources, and prompts modules behind stable crate-root exports"
  - "Full compatibility validation after staged compiler and MCP splits"
affects: [ctxhelm-compiler, ctxhelm-mcp, cli, mcp, public-api]
...
key-decisions:
  - "Kept ctxhelm-compiler and ctxhelm-mcp crate roots as stable public facades while moving implementation into concern-focused modules."
...
- Split `ctxhelm-compiler` into `planning`, `packs`, `cards`, and `eval` modules with stable public exports from `ctxhelm_compiler`.
- Split `ctxhelm-mcp` into `protocol`, `schemas`, `tools`, `resources`, and `prompts` modules with stable `run_stdio_server` and `run_server` exports.
...
- `crates/ctxhelm-compiler/src/lib.rs` - Stable compiler facade and existing crate-local tests.
- `crates/ctxhelm-compiler/src/planning.rs` - Context planning, ranking, anchors, confidence, and low-information helpers.
- `crates/ctxhelm-compiler/src/packs.rs` - Context pack compilation, Markdown rendering, snippets, and checklist rendering.
- `crates/ctxhelm-compiler/src/cards.rs` - Source-free context card generation and rendering helpers.
- `crates/ctxhelm-compiler/src/eval.rs` - Historical eval reports, eval traces, parent snapshot extraction, and metrics.
- `crates/ctxhelm-mcp/src/lib.rs` - Stable MCP facade, tool-name constants, and compatibility tests.
- `crates/ctxhelm-mcp/src/protocol.rs` - JSON-RPC protocol loop, request dispatch, and error responses.
- `crates/ctxhelm-mcp/src/schemas.rs` - Initialize, tools/list, resources/list, and prompts/list schema payloads.
- `crates/ctxhelm-mcp/src/tools.rs` - MCP tool handlers and tool argument contracts.
- `crates/ctxhelm-mcp/src/resources.rs` - MCP resource reads, repo/file/symbol resources, pack guide, and session pack cache.
- `crates/ctxhelm-mcp/src/prompts.rs` - MCP prompt handlers and workflow prompt rendering.
...
- **Files modified:** `crates/ctxhelm-compiler/src/*.rs`, `crates/ctxhelm-mcp/src/*.rs`
- **Verification:** `cargo test -p ctxhelm-compiler`, `cargo test -p ctxhelm-mcp`, and final workspace validation passed.
...
- `cargo test -p ctxhelm-mcp` passed.
- `cargo test -p ctxhelm --test cli_compat serve_mcp -- --nocapture` passed.
- `cargo test --workspace` passed.
- `cargo run -p ctxhelm -- --help` passed and listed `serve-mcp`.
