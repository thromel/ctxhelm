...
must_haves:
  ...
  key_links:
    - from: "crates/ctxpack-mcp/src/lib.rs"
      to: "crates/ctxpack-core/src/contracts.rs"
      via: "structuredContent serializes ContextPlan and ContextPack"
      pattern: "structuredContent"
    - from: "crates/ctxpack-compiler/src/lib.rs"
      to: "crates/ctxpack-core/src/contracts.rs"
...
<task type="auto" tdd="true">
  ...
  <files>crates/ctxpack-core/src/contracts.rs, crates/ctxpack-compiler/src/lib.rs</files>
  <read_first>
    - crates/ctxpack-core/src/contracts.rs
    - crates/ctxpack-compiler/src/lib.rs
  ...
  <acceptance_criteria>
    - `rg -n 'context_plan_public_json_shape_is_stable|context_pack_public_json_shape_is_stable|eval_trace_public_json_shape_is_source_free' crates/ctxpack-core/src/contracts.rs` finds all three tests.
    - `rg -n 'historical_eval_report_public_json_shape_is_stable|evaluatedCommits|fileRecallAt5|lexicalBaselineRecallAt5|topMissingFiles|privacyStatus' crates/ctxpack-compiler/src/lib.rs` finds all report fields.
    - `cargo test -p ctxpack-core public_json_shape -- --nocapture` passes.
    - `cargo test -p ctxpack-compiler public_json_shape -- --nocapture` passes.
  </acceptance_criteria>
...
<task type="auto" tdd="true">
  ...
  <files>crates/ctxpack-mcp/src/lib.rs</files>
  <read_first>
    - crates/ctxpack-mcp/src/lib.rs
    - crates/ctxpack-core/src/contracts.rs
    - crates/ctxpack-compiler/src/lib.rs
  ...
  <behavior>
    - `initialize` returns `serverInfo.name == "ctxpack"` and tools/resources/prompts capabilities with `listChanged: false`.
    ...
    - `resources/list` asserts the current static list only: `ctxpack://repo/summary`, `ctxpack://repo/test-map`, `ctxpack://repo/dependency-graph`, and `ctxpack://pack/guide`.
    - `resources/read` covers static repo resources plus dynamic `ctxpack://file/...`, `ctxpack://symbol/...`, and same-session generated `ctxpack://pack/...` reads.
  ...
  <action>
    ...
    Per D-09, assert `resources/list` returns only the current static descriptors: `ctxpack://repo/summary`, `ctxpack://repo/test-map`, `ctxpack://repo/dependency-graph`, and `ctxpack://pack/guide`.

    Use `resources/read` tests to characterize repo resources, dynamic `ctxpack://file/...` resources, dynamic `ctxpack://symbol/...` resources, and generated `ctxpack://pack/...` resources available only within the same server process; do not implement cross-process durability.
  </action>
  <verify>
    <automated>cargo test -p ctxpack-mcp public_surface -- --nocapture</automated>
    <automated>cargo test -p ctxpack-mcp session_scoped -- --nocapture</automated>
    <automated>cargo test -p ctxpack-mcp error_codes -- --nocapture</automated>
  </verify>
  <acceptance_criteria>
    - `rg -n 'initialize_public_capabilities_are_stable|tools_list_public_surface_is_exact|tool_calls_include_text_and_structured_content|resources_public_uri_shapes_are_stable|prompts_public_surface_is_stable|pack_resources_are_session_scoped_characterization|json_rpc_error_codes_are_stable' crates/ctxpack-mcp/src/lib.rs` finds all seven tests.
    - `rg -n 'prepare_task.*search.*related.*get_pack.*related_tests.*current_diff|structuredContent.*\\[0\\]|content\\]\\[0\\]|ctxpack://repo/summary|ctxpack://repo/test-map|ctxpack://repo/dependency-graph|ctxpack://pack/guide|ctxpack://file|ctxpack://symbol|ctxpack://pack|listChanged' crates/ctxpack-mcp/src/lib.rs` finds the public-surface assertions.
    - `cargo test -p ctxpack-mcp public_surface -- --nocapture` passes.
    - `cargo test -p ctxpack-mcp session_scoped -- --nocapture` passes.
    - `cargo test -p ctxpack-mcp error_codes -- --nocapture` passes.
  </acceptance_criteria>
...
<verification>
Run the focused commands above, then run `cargo test -p ctxpack-core -p ctxpack-compiler -p ctxpack-mcp`.
</verification>
