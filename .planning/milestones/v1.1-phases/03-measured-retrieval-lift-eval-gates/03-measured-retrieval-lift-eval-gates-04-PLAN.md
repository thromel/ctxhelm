...
must_haves:
  truths:
    - "Historical eval compares ctxpack ranking, lexical-only ranking, and signal ablations over the same frozen commits and same budget."
  ...
  key_links:
    - from: "crates/ctxpack-compiler/src/eval.rs"
...
Purpose: Maintainers need to see whether ctxpack beats lexical retrieval at fixed budgets and, if not, which signal families failed.
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 2: Report includes Recall@K, Precision@K, MRR@K, ctxpack lift, role-aware recall, and test recommendation rate.
  ...
  <verify>
    <automated>cargo test -p ctxpack-compiler ranking_metrics -- --nocapture</automated>
  </verify>
...
<task type="auto" tdd="true">
  ...
  <files>crates/ctxpack-compiler/src/eval.rs, crates/ctxpack-compiler/src/lib.rs, crates/ctxpack/src/main.rs</files>
  <behavior>
  ...
  <verify>
    <automated>cargo test -p ctxpack-compiler ablation -- --nocapture && cargo test -p ctxpack historical_eval_report -- --nocapture && cargo tree --workspace --depth 1 > /tmp/ctxpack-phase3-cargo-tree.txt && ! rg "tantivy|rayon|rusqlite|notify|tree-sitter|mcp-sdk" /tmp/ctxpack-phase3-cargo-tree.txt</automated>
  </verify>
...
<task type="auto" tdd="true">
  ...
  <files>crates/ctxpack-compiler/src/eval.rs, crates/ctxpack/src/main.rs</files>
  <behavior>
  ...
  <verify>
    <automated>cargo test -p ctxpack eval_checklist -- --nocapture</automated>
  </verify>
