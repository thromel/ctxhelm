...
must_haves:
  truths:
    ...
    - "README quickstart commands use the installed `ctxhelm` binary, current CLI flags, and explicit `--repo` arguments."
  ...
  key_links:
    ...
    - from: "docs/quickstart.md"
      to: "ctxhelm prepare-task/get-pack"
      via: "installed-binary commands with explicit repo"
      pattern: "ctxhelm prepare-task.*--repo|ctxhelm get-pack.*--repo"
---
...
- Phase 6 decision: First-pack user path uses `ctxhelm`/`CTXHELM_BIN`, explicit `--repo`, `setup-check`, deterministic MCP proof, `prepare-task`, and `get-pack`.
...
<task type="auto">
  ...
  <action>Update README so the first user-facing path after install is a concise "Install to first pack" flow using the installed `ctxhelm` binary, not `cargo run`. Keep the existing development commands in a clearly separate Development section. The quickstart must show: `ctxhelm --version`, `ctxhelm --help`, choose a repo path, `ctxhelm init --repo "$REPO" --cursor --claude --opencode`, `ctxhelm setup-check --repo "$REPO" --cursor --claude --opencode`, `ctxhelm prepare-task "..." --repo "$REPO" --mode bug-fix --path ...`, and `ctxhelm get-pack "..." --repo "$REPO" --mode bug-fix --budget brief`. Link to `docs/release.md` for install/release details and to `docs/quickstart.md` for the longer first-pack walkthrough. Remove README examples that present `cargo run -p ctxhelm -- init`, `cargo run -p ctxhelm -- serve-mcp`, or other source-checkout commands as the normal user setup path; leave source-build commands only in development/fallback contexts.</action>
  <verify>
    <automated>python3 -c "from pathlib import Path; r=Path('README.md').read_text(); assert 'docs/quickstart.md' in r; assert 'ctxhelm init --repo' in r; assert 'ctxhelm setup-check --repo' in r; assert 'ctxhelm prepare-task' in r; assert 'ctxhelm get-pack' in r; assert 'cargo run -p ctxhelm -- init' not in r; assert 'cargo run -p ctxhelm -- serve-mcp' not in r"</automated>
  </verify>
...
<verification>
- `python3 -c "from pathlib import Path; r=Path('README.md').read_text(); q=Path('docs/quickstart.md').read_text(); assert 'ctxhelm init --repo' in r and 'ctxhelm get-pack' in q"`
- `cargo run -p ctxhelm -- --help`
</verification>

<success_criteria>
Plan 01 is complete when DOCS-01 is satisfied by README and `docs/quickstart.md`: users can install or verify `ctxhelm`, initialize a repo, validate setup, and request a first plan/pack with installed-binary commands and explicit repo arguments.
</success_criteria>
