...
must_haves:
  truths:
    - "User can troubleshoot PATH failures, absolute MCP binary paths, CTXHELM_HOME/state cleanup, wrong-cwd behavior, and common MCP startup failures from docs."
  ...
  key_links:
    ...
    - from: "scripts/check-release-docs.sh"
      to: "docs/troubleshooting.md"
      via: "grep checks for PATH, CTXHELM_HOME, wrong-cwd, setup-check, and pack resource caveats"
      pattern: "CTXHELM_HOME|wrong cwd|setup-check|session-scoped"
---
...
<decision_trace>
- DOCS-02: Docs must cover install methods, PATH failures, absolute MCP binary paths, `CTXHELM_HOME`, uninstall/state cleanup, wrong-cwd behavior, and common MCP startup failures.
...
<task type="auto">
  ...
  <action>Create `docs/troubleshooting.md` with concrete symptoms, likely causes, and fixes for: `ctxhelm: command not found`, GUI/agent PATH differences, using an absolute `ctxhelm` binary path in MCP config, verifying `ctxhelm --version` and `ctxhelm --help`, `CTXHELM_HOME` location and isolated temp homes, uninstall and state cleanup (`~/.ctxhelm` or custom home), wrong-cwd behavior and explicit `--repo`/MCP `repo`, common stdio MCP startup failures, stdout cleanliness expectations, permissions/read-only home behavior, `setup-check` scope, and same-session/session-scoped pack resource caveats with `get_pack` as the durable reconnect path. Keep examples source-safe and local-only; do not tell users ctxhelm runs their project tests or mutates global agent config.</action>
  <verify>
...
<task type="auto" tdd="true">
  ...
  <files>README.md, scripts/check-release-docs.sh, crates/ctxhelm/tests/release_packaging.rs</files>
  <behavior>
    ...
    - Test 3: Docs checker requires installed-binary first-pack commands: `ctxhelm init --repo`, `ctxhelm setup-check --repo`, `ctxhelm prepare-task`, and `ctxhelm get-pack`.
    - Test 4: Docs checker requires troubleshooting coverage for PATH, absolute binary paths, `CTXHELM_HOME`, wrong cwd, MCP startup, setup-check scope, uninstall/state cleanup, and session-scoped pack resources.
