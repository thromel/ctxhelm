...
Expose a credible source-free product proof path for v1.2: a maintainer command that runs a configured benchmark suite, summarizes headline metrics, states limitations and where ctxhelm helps or does not help, and optionally runs inside the release gate.
...
The release gate should only run benchmark proof when `CTXHELM_BENCHMARK_CONFIG` is set. This keeps normal release verification deterministic and does not require local copies of RefactoringMiner or another external repository.
...
- `ctxhelm eval benchmark` and `ctxhelm eval compare` provide the underlying reproducible command paths.
...
- Add `ctxhelm eval proof --config <suite.json>`.
...
- Add optional `CTXHELM_BENCHMARK_CONFIG` release-gate proof.
...
- Hard release-gate thresholds for benchmark regressions can be configured later on top of `ctxhelm eval compare`.
