...
- [x] **Phase 10: Fixed-Budget Retrieval Metrics & Baselines** - Maintainers can compare ctxhelm retrieval against lexical and no-context baselines with stable metrics.
...
- [x] **Phase 12: Product Proof Report & Adoption Gate** - Users can see why ctxhelm is useful through reproducible reports, docs, and release-gate proof.
...
**Goal**: Maintainers can measure whether ctxhelm improves target-file/test retrieval over lexical and no-context baselines at fixed budgets.
**Depends on**: Phase 9
...
**Success Criteria** (what must be TRUE):
  ...
  2. Reports compare ctxhelm hybrid ranking against lexical-only and no-context/anchor-only baselines under the same candidate and token budgets.
...
**Goal**: Users can see a credible, reproducible answer to "why use ctxhelm instead of the agent's native search?"
**Depends on**: Phase 11
...
**Success Criteria** (what must be TRUE):
  ...
  2. `ctxhelm eval` exposes a maintainer-friendly command path that reproduces the product proof on configured local repos.
  3. Release gate can optionally run a bounded benchmark smoke and fail on report-generation or privacy regressions.
  4. The proof report states when ctxhelm helps, when it does not, and what future milestones address remaining gaps.
...
- [x] 12-product-proof-report-adoption-gate-02-PLAN.md — Document benchmark setup, interpretation, limitations, and "why ctxhelm" messaging.
