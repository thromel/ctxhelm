...
This phase turns ctxpack's typed retrieval candidates into source-free feature rows that can be stored, inspected, compared, and used by later paired-baseline and learned-policy phases.
...
- Store exports in local ctxpack state under the repo ID, not in the repository by default.
...
- `ctxpack eval feedback` already records read/edit/test/correction labels for future feedback joins.
- Phase 51 added local source-free report cache storage conventions under ctxpack home.
...
- Add `CandidateFeatureExport` and `CandidateFeatureRow` contracts to `ctxpack-core`.
- Add `ctxpack eval features export/list/inspect/compare/delete`.
