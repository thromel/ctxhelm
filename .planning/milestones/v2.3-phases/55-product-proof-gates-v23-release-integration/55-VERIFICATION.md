...
- [x] `CARGO_INCREMENTAL=0 cargo test -p ctxhelm --test cli_compat eval_proof_generates_source_free_product_report`
- [x] `CARGO_INCREMENTAL=0 cargo test -p ctxhelm --test release_packaging release_gate_script_contract`
- [x] `bash scripts/check-release-docs.sh`
- [x] `CARGO_INCREMENTAL=0 cargo run -p ctxhelm -- eval proof --help`
- [x] `CTXHELM_BIN="$(pwd)/target/debug/ctxhelm" bash scripts/smoke-v23-eval.sh`
