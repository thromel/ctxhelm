...
# Requirements: ctxhelm v1.2 Retrieval Quality Proof
...
Requirements for proving ctxhelm's product value through repeatable, source-free real-repo retrieval benchmarks.
...
- [x] **METR-02**: Benchmark reports compare ctxhelm hybrid ranking against lexical-only and no-context or anchor-only baselines under fixed budgets.
...
- [x] **PROOF-02**: `ctxhelm eval` exposes a maintainer-friendly command path that reproduces the proof report on configured local repos.
...
- [x] **PROOF-04**: Product documentation clearly states when ctxhelm helps, when it does not, and how agents should use the evidence.
...
- **STORE-03**: Add schema versioning, migrations, repair, and cleanup commands for local ctxhelm state.
...
| Feature | Reason |
|---------|--------|
| New autonomous editing behavior | ctxhelm remains a read-only context broker; agents own edits and permissions. |
