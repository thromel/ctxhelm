...
**Domain:** ctxhelm v1.1 Packaging & Adoption
...
**What goes wrong:** Users follow the README, but the installed `ctxhelm` binary is missing, not on `PATH`, points MCP clients at `cargo run`, or only works from this repository.
...
**Consequences:** First-run adoption fails with `spawn ctxhelm ENOENT`, stale source-checkout paths in MCP config, or unreproducible dependency resolution.
...
**Detection:** A release gate should run `ctxhelm --version`, `ctxhelm --help`, `ctxhelm serve-mcp` handshake, `ctxhelm init`, and MCP smoke from outside the repo with a stripped-down `PATH`.
...
**Why it happens:** Agent clients have different cwd and project-root behavior. Claude Code sets `CLAUDE_PROJECT_DIR` for stdio servers, but ctxhelm already validated that explicit `repo` is the safer contract across clients.
**Consequences:** ctxhelm recommends files from the wrong checkout, creates cache entries for the wrong repo, or looks broken even though the server is healthy.
**Prevention:** Keep every v1.1 setup guide and adapter instruction repo-explicit: call `prepare_task` and `get_pack` with `repo`. `ctxhelm init` should show the active repo path and generated snippets should avoid relying on cwd.
...
**Detection:** Release CI should support `CTXHELM_SKIP_REAL_CLIENT=1` for portable protocol proof and `CTXHELM_REQUIRE_REAL_CLIENT=1` for maintainer machines before tagging.

### Pitfall 4: PATH and Shell Differences Are Hidden Until Runtime

**What goes wrong:** MCP config uses `ctxhelm`, but the GUI/agent process cannot find it because it has a different login shell, launch environment, or PATH than the user's terminal.
...
**Prevention:** `ctxhelm init` should resolve and print the absolute binary path used in snippets. Add troubleshooting for `which ctxhelm`, `ctxhelm --version`, and replacing `command: "ctxhelm"` with the absolute path.
...
**Detection:** Add tests or a script that greps docs for old `cargo run -p ctxhelm -- serve-mcp` production snippets, obsolete client versions, and missing `repo` guidance.

### Pitfall 6: Packaging Secrets, Caches, or Local State

**What goes wrong:** Release artifacts, tarballs, generated cards, docs examples, or CI uploads include `.ctxhelm`, `~/.ctxhelm`-style caches, traces, real repo paths, tokens, request logs, or local MCP configs.
...
**Prevention:** Release packaging must start from a clean checkout and explicit include list. Exclude `.ctxhelm/`, `.codex/`, `.claude/`, temp smoke logs, traces, caches, target artifacts unless intentionally packaging binaries, and all local config files.
**Detection:** Add an artifact audit step that lists archive contents and fails on `.ctxhelm`, `traces.jsonl`, `.env`, key/token-looking filenames, absolute `/Users/` paths, and request-log files.
...
**Prevention:** Add a v1.1 release checklist or script that runs from a clean temp home: build/package, install into temp prefix, verify `PATH`, run `ctxhelm init`, run deterministic MCP smoke, run docs command snippets where practical, and audit artifacts.
...
**Prevention:** Keep generated snippets repo-local or copy/paste-oriented. For smoke tests, use temp homes/configs only. Document exactly which files `ctxhelm init` writes.

### Pitfall 2: Confusing "Configured" With "Used"

**What goes wrong:** A docs step passes after `codex mcp list` or `claude mcp get`, but the agent never calls ctxhelm.
...
**Prevention:** Package `README`, license metadata, and `ctxhelm --version`; tag release artifacts with version, commit, target triple, and checksum.
...
2. Install into a temp prefix and verify `ctxhelm --version` and `ctxhelm --help`.
3. Run `ctxhelm init` in a fixture repo and verify generated guidance uses explicit `repo`.
...
- Local project context: `.planning/PROJECT.md`, `.planning/STATE.md`, `README.md`, `scripts/smoke-mcp-protocol.sh`, `scripts/smoke-codex-mcp.sh`, `scripts/smoke-claude-mcp.sh` - HIGH confidence for current ctxhelm v1.1 scope and existing smoke behavior.
