...
**Project:** ctxpack / Repo Context Packer
...
|------|----------------|------------------|
| Binary package | `crates/ctxpack` exposes one binary named `ctxpack`; workspace packages are still `0.1.0`. | Release should make `ctxpack` the only user-installed binary and bump package/tag version together. |
...
| Runtime surface | MCP is launched as `ctxpack serve-mcp`; generated adapters already assume `ctxpack` is on `PATH`. | Packaging must prioritize reliable `PATH` installation and MCP stdio cleanliness. |
| Local state | `CTXPACK_HOME` overrides state; default is `$HOME/.ctxpack`, falling back to repo-local `.ctxpack` if `HOME` is unavailable. | Install docs must explain state location and env forwarding for MCP clients. |
...
| Platform archives | `ctxpack-v1.1.0-{target}.tar.gz` for macOS/Linux, `.zip` for Windows | Manual install path | Developers can inspect/extract a single binary. This is simpler and more privacy-transparent than making package managers the first path. |
...
| Release smoke scripts | Repo scripts plus an installed-binary variant | Publish gate | Keep existing protocol and real-client gates, but run at least one smoke against `./ctxpack serve-mcp` extracted from the release archive. |
...
| CLI version output | Add Clap version metadata so `ctxpack --version` returns the package version. This becomes the first support diagnostic. |
...
|------|----------------|--------------|-------|
| Prebuilt archive | Download, verify checksum, extract `ctxpack`, move to a directory on `PATH`. | Yes, primary | Most transparent path; no Rust toolchain required. |
...
| `cargo install --git` | `cargo install --git <repo-url> --tag v1.1.0 ctxpack --locked` | Yes, fallback | Works for source builds; document that Rust is required and `--locked` uses the committed lockfile. |
| Build from checkout | `cargo install --path crates/ctxpack --locked` or `cargo build -p ctxpack --release --locked` | Yes, developer fallback | Good for contributors and unsupported platforms. |
| crates.io | `cargo install ctxpack --locked` | Defer | Current manifests are not crates.io-ready: path-only internal deps, sparse package metadata, and no release need to publish all internal crates yet. |
...
- Installation docs should verify `ctxpack --version` and `ctxpack --help` before any MCP setup.
- MCP setup snippets should continue to call `ctxpack serve-mcp`, but troubleshooting must explain that the MCP client may have a different `PATH` than the user's shell.
- For robust client configs, recommend an absolute binary path when users hit PATH issues, for example `/Users/name/.local/bin/ctxpack` or `%USERPROFILE%\.local\bin\ctxpack.exe`.
- Document `CTXPACK_HOME` as the only supported state override. Default state remains `$HOME/.ctxpack`; examples should set `CTXPACK_HOME` only for tests, CI, or isolated client smoke.
- Do not make installers mutate Codex, Claude, Cursor, or OpenCode global config. Keep `ctxpack init --repo ... --claude --cursor --opencode` as the repo-local setup step.
...
| Read-only product role | Installers may place the `ctxpack` binary; ctxpack may write its own caches, traces, cards, and adapter files, but must not edit source code or agent global configs. |
| Source-safe release assets | Release artifacts should include the binary, README, LICENSE, and completion/docs if added. Do not bundle `.ctxpack`, traces, local inventories, test fixtures with secrets, or developer target directories. |
| Stdio MCP safety | `ctxpack serve-mcp` must keep stdout reserved for JSON-RPC. Diagnostics belong on stderr or structured MCP responses. |
| Trust diagnostics | The first-run guide should state where local state is written and how to delete it: remove `$CTXPACK_HOME` or `$HOME/.ctxpack`. |

## Minimal v1.1 Implementation Sequence

1. Add release metadata: root `LICENSE`, package `description`, `repository`, `readme`, `rust-version`, and `ctxpack --version`.
2. Configure `dist` for the single `ctxpack` binary, archive artifacts, GitHub Releases, and sha256 checksums.
3. Add release CI gates: `cargo test --workspace`, `cargo run -p ctxpack -- --help`, `scripts/smoke-mcp-protocol.sh`, and an extracted-binary `ctxpack serve-mcp` smoke.
...
5. Update adapter/setup docs to cover PATH failures, absolute binary paths, explicit `repo` arguments, and `CTXPACK_HOME`.
...
- Repo inspection: `Cargo.toml`, `crates/ctxpack/Cargo.toml`, `README.md`, `crates/ctxpack/src/main.rs`, `crates/ctxpack-core/src/init.rs`, `crates/ctxpack-index/src/inventory.rs`, `scripts/smoke-*.sh`. Confidence: HIGH.
...
- cargo-dist docs: https://axodotdev.github.io/cargo-dist/ and https://axodotdev.github.io/cargo-dist/book/artifacts/checksums.html. Confidence: MEDIUM until exercised in ctxpack CI.
