...
<action>Create a storage privacy test fixture with a temporary git repo containing a safe source file whose body includes unique sentinel strings such as `CTXHELM_SHOULD_NOT_STORE_SOURCE_BODY` and `CTXHELM_SHOULD_NOT_STORE_PROMPT_TEXT`. Initialize the store and insert or sync only source-free metadata supported by Phase 13.</action>
<acceptance_criteria>
- Test fixture writes a source file containing `CTXHELM_SHOULD_NOT_STORE_SOURCE_BODY`.
- Test fixture includes a simulated prompt/task sentinel `CTXHELM_SHOULD_NOT_STORE_PROMPT_TEXT` only in test input, not in expected persisted fields.
...
<acceptance_criteria>
- Test fails if `CTXHELM_SHOULD_NOT_STORE_SOURCE_BODY` appears in database bytes.
- Test fails if `CTXHELM_SHOULD_NOT_STORE_PROMPT_TEXT` appears in database bytes.
...
- `cargo test -p ctxhelm-index write_inventory_persists_under_ctxhelm_home` still passes.
</acceptance_criteria>
</task>
...
<acceptance_criteria>
- `cargo test -p ctxhelm-index storage` exits 0.
