...
This roadmap hardens ctxpack from a useful post-MVP context broker into a trustworthy, measurable agent-native product. Phases 1-4 completed the v1 foundation: public contract guardrails, trust diagnostics, measured retrieval, and real-client durability. Phases 5-8 continue with v1.1 Packaging & Adoption: release artifacts, agent setup, documentation, and publish gates that let developers install ctxpack, wire it into agents, and prove it works without expanding the product beyond local, read-only context delivery.
...
- [x] **Phase 5: Release Identity & Binary Packaging** - Users can install a versioned ctxpack binary from audited release artifacts without a source checkout.
- [x] **Phase 6: Agent Setup & First-Pack Adoption** - Users can initialize repo-local agent guidance and complete the first useful ctxpack flow.
...
**Goal**: Users can install and verify a versioned ctxpack binary from repeatable, audited release artifacts without needing a source checkout.
**Depends on**: Phase 4
**Requirements**: PKG-01, PKG-02, PKG-03, PKG-04, PKG-05
**Success Criteria** (what must be TRUE):
  1. User can download or extract a versioned `ctxpack` binary and confirm `ctxpack --version` and `ctxpack --help` without a source checkout.
  ...
  5. Maintainer can audit release archive contents and confirm ctxpack local state, traces, logs, temp homes, secrets, absolute local paths, and unintended caches are absent.
**Plans**: 4 plans
Plans:
...
**Goal**: Users can initialize repo-local agent setup guidance and complete the first useful ctxpack context flow through the agent-native MCP-first surface.
**Depends on**: Phase 5
**Requirements**: ADPT-01, ADPT-02, ADPT-03, ADPT-04, ADPT-05
**Success Criteria** (what must be TRUE):
  1. User can run `ctxpack init` and see a clear report of repo-local files written, skipped, or left unchanged, plus exact next setup and smoke steps.
  2. User can use generated Codex CLI, Claude Code, Cursor, and OpenCode guidance without ctxpack mutating global agent configuration by default.
...
**Plans**: 4 plans
Plans:
- [x] 06-agent-setup-first-pack-adoption-01-PLAN.md — Make `ctxpack init` report file actions and exact next steps.
...
- [x] 06-agent-setup-first-pack-adoption-04-PLAN.md — Add deterministic first-pack adoption smoke using a selected ctxpack binary.
...
**Success Criteria** (what must be TRUE):
  ...
  2. User can troubleshoot PATH failures, absolute MCP binary paths, `CTXPACK_HOME`, uninstall or state cleanup, wrong-cwd behavior, and common MCP startup failures.
...
**Success Criteria** (what must be TRUE):
  1. Maintainer can run a release gate that smokes the installed or extracted `ctxpack` binary instead of only `cargo run`.
  2. Deterministic MCP protocol smoke can run from a wrong cwd with an explicit repo and selectable `CTXPACK_BIN` or equivalent binary path.
