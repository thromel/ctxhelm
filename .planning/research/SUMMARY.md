...
**Scope:** Make the completed v1 ctxpack product installable, configurable, documented, and smoke-testable without changing the retrieval engine or product boundaries.
...
> Can a developer install ctxpack, wire it into an agent, prove MCP works, and get a first useful context pack from a real repository?

## Recommended v1.1 Shape

1. **Release identity and packaging**
   - Bump workspace versions consistently before publishing a v1.1 tag.
   - Add `ctxpack --version` as the first support diagnostic.
...
2. **Installed-binary smoke gates**
   - Release gates must smoke the installed or extracted `ctxpack` binary, not only `cargo run`.
   - Required gates should include `ctxpack --help`, `ctxpack --version`, `ctxpack init`, deterministic MCP protocol smoke from the wrong cwd, and artifact audits.
...
4. **Docs and troubleshooting**
   ...
   - Cover install, PATH, absolute binary paths, `CTXPACK_HOME`, wrong-cwd MCP behavior, pack-resource session scope, generated files, smoke commands, and uninstall/state cleanup.
...
5. **Privacy and release audit**
   - Release artifacts must exclude `.ctxpack`, traces, request logs, local MCP configs, temp homes, secrets, and absolute local paths.
...
| MCP client cannot find `ctxpack` on PATH | Document absolute binary paths and make generated guidance diagnose PATH issues. |
