...
Do not introduce a hosted service, background daemon, editor plugin, or autonomous setup writer for v1.1. The release should distribute one `ctxpack` binary and repo-local adapter artifacts; MCP clients launch `ctxpack serve-mcp`.

## Component Boundaries

| Component | Responsibility | v1.1 Impact |
|-----------|----------------|-------------|
| `crates/ctxpack/src/main.rs` | CLI entry point and `serve-mcp` launcher. | Add only packaging-safe commands or flags if required by install/smoke flows; keep command output contract-tested. |
| `crates/ctxpack-core/src/init.rs` | `AGENTS.md`, `.ctxpack/ctxpack.toml`, and adapter templates. | Treat adapter text as product contract. Add new setup artifacts here, not in CLI string literals. |
| `crates/ctxpack-mcp/src/*` | MCP stdio protocol, tools, resources, prompts, session cache. | Preserve existing tool/resource names and structuredContent shapes. No stdout logging. |
...
| `scripts/smoke-codex-mcp.sh`, `scripts/smoke-claude-mcp.sh` | Optional real-client smokes with required mode. | Keep optional by default; CI/release can set `CTXPACK_REQUIRE_REAL_CLIENT=1` only on machines with clients/auth. |
...
If v1.1 adds setup helpers, make them generated artifacts or validation commands, not client-specific global mutators. This preserves the product rule that ctxpack brokers context but does not take over agent configuration or editing authority.
...
docs/troubleshooting.md           # PATH, wrong repo cwd, CTXPACK_HOME, MCP stdout, stale cache
docs/contracts.md                 # stable CLI/MCP/JSON compatibility notes
```

Avoid long static repository maps in docs and adapters. The docs should teach users to invoke dynamic ctxpack calls, not paste ctxpack output into rule files.
...
| Tier | Gate | Why |
|------|------|-----|
| Deterministic binary smoke | `ctxpack --help`, `ctxpack init`, `ctxpack prepare-task --no-trace`, `ctxpack get-pack --format json --no-trace` on a fixture repo. | Proves installed binary, CLI contract, and repo-local writes. |
| Deterministic MCP protocol smoke | `scripts/smoke-mcp-protocol.sh` against installed `ctxpack serve-mcp`. | Proves stdio JSON-RPC behavior without client auth/model variability. |
...
The current protocol smoke uses `cargo run`; v1.1 should allow `CTXPACK_BIN=/path/to/ctxpack` or equivalent so packaging can test the artifact users will run.
...
- Repo-local writes are limited to ctxpack-owned setup/cache/card artifacts already allowed by product rules.
...
Keep all generated agent guidance in `ctxpack-core::init`, covered by tests for size, dynamic wording, explicit `repo`, and no static context dumps.
...
|--------------|---------|---------|
| Release scripts call `cargo run` only | Tests the source tree, not the installed product. | Smoke the packaged `ctxpack` binary. |
...
| Support burden | Put PATH, explicit `repo`, wrong cwd, `CTXPACK_HOME`, and stdout logging failures in troubleshooting docs. |
...
- Local: `crates/ctxpack-core/src/init.rs`, `crates/ctxpack/src/main.rs`, `scripts/smoke-*.sh`, `crates/ctxpack/tests/cli_compat.rs`
