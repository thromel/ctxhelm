...
mod tests {
    ...
    fn context_area_resource_uri_round_trips_source_free_area_names() {
        ...
        assert_eq!(uri, "ctxhelm://repo/context-area/src%2Fauth");
        assert_eq!(
            decode_context_area_resource_uri(&uri).as_deref(),
            Some("src/auth")
        );
        assert_eq!(
            decode_context_area_resource_uri("ctxhelm://repo/context-area/%ZZ"),
            None
        );
    }
    ...
    fn context_plan_public_json_shape_is_stable() {
        ...
        let expected = serde_json::json!({
            ...
            "packOptions": [{
                "budget": "brief",
                "resourceUri": "ctxhelm://packs/brief"
            }],
    ...
    fn diagnostics_public_json_shape_is_source_free_and_backward_compatible() {
        let diagnostic = Diagnostic {
            ...
            paths: vec![".ctxhelm/repos/repo-1/inventory.json".to_string()],
            count: 1,
        };
    ...
    fn workspace_contracts_are_camel_case_and_source_free() {
        ...
        let report = WorkspaceInventoryReport {
            ...
            manifest_path: "/tmp/workspace/.ctxhelm/workspace.json".to_string(),
            repo_count: 1,
    ...
    fn shared_artifact_and_team_policy_contracts_are_source_free() {
        let artifact = SharedArtifactEntry {
            ...
            path_label: ".ctxhelm/feedback-summary.json".to_string(),
        ...
        let report = TeamPolicyReport {
            policy_path: ".ctxhelm/team-policy.json".to_string(),
            policy,
