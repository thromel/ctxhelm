...
use ctxhelm_index::{task_hash, InventoryError, SemanticProviderConfig};
use std::path::Path;
...
fn mcp_resources_for_plan(
    ...
    let mut resources = vec![
        "ctxhelm://repo/summary".to_string(),
        "ctxhelm://repo/test-map".to_string(),
        "ctxhelm://repo/dependency-graph".to_string(),
        "ctxhelm://repo/memory".to_string(),
        "ctxhelm://repo/context-areas".to_string(),
    ...
    for target in plan.target_files.iter().take(3) {
        resources.push(format!("ctxhelm://file/{}", target.path));
    }
    resources
}
...
fn guidance_surfaces(target_agent: &str) -> Vec<AgentPreviewSurface> {
    let mut surfaces = vec![AgentPreviewSurface {
        ...
        label: "AGENTS.md ctxhelm policy".to_string(),
    ...
    match target_agent {
        "codex" => surfaces.push(AgentPreviewSurface {
            ...
            path: None,
            summary:
                "User-owned Codex MCP config can launch `ctxhelm serve-mcp`; ctxhelm does not mutate global config."
                    .to_string(),
        }),
        "claude-code" => surfaces.push(AgentPreviewSurface {
            ...
            path: Some(".claude/commands/ctxhelm-bugfix.md".to_string()),
            summary:
        ...
        "cursor" => surfaces.push(AgentPreviewSurface {
            ...
            path: Some(".cursor/rules/ctxhelm.mdc".to_string()),
            summary:
                "Project rule steers Cursor toward ctxhelm MCP planning while leaving file reads and edits to Cursor."
                    .to_string(),
        }),
        "opencode" => surfaces.push(AgentPreviewSurface {
            kind: AgentPreviewSurfaceKind::AdapterSnippet,
            label: "OpenCode MCP snippet".to_string(),
            path: Some(".ctxhelm/adapters/opencode.jsonc.snippet".to_string()),
            summary:
                "Repo-local snippet can be reviewed and merged by the user; ctxhelm does not mutate OpenCode config."
                    .to_string(),
        }),
        "generic" => surfaces.push(AgentPreviewSurface {
            ...
            path: Some("ctxhelm://guides/context-pack".to_string()),
            summary:
        ...
        _ => surfaces.push(AgentPreviewSurface {
            ...
            path: None,
            summary:
                "Custom agents can call `ctxhelm prepare-task` and `ctxhelm get-pack` when MCP is unavailable."
                    .to_string(),
        }),
    }
...
fn native_rule_surfaces(target_agent: &str) -> Vec<AgentPreviewSurface> {
    match target_agent {
        "claude-code" => vec![AgentPreviewSurface {
            ...
            path: Some(".ctxhelm/adapters/claude-mcp.json".to_string()),
            summary: "Mergeable project MCP snippet for local stdio ctxhelm.".to_string(),
        }],
...
fn next_steps(target_agent: &str) -> Vec<AgentPreviewStep> {
    vec![
        AgentPreviewStep {
            order: 1,
            action: "Call ctxhelm.prepare_task with the active repo path.".to_string(),
        ...
        AgentPreviewStep {
            order: 3,
            action: "Call ctxhelm.get_pack with brief or standard budget only if more context is needed."
                .to_string(),
            owner: target_agent.to_string(),
            source_bearing: true,
        },
...
fn boundary_lines() -> Vec<String> {
    vec![
        "ctxhelm is read-only and does not edit source files.".to_string(),
        "ctxhelm suggests target files, related tests, context packs, and validation commands."
            .to_string(),
