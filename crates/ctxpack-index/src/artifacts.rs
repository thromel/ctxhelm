...
pub fn team_policy_path(repo_root: impl AsRef<Path>) -> PathBuf {
    repo_root.as_ref().join(".ctxpack").join("team-policy.json")
}
...
fn build_shared_artifact_manifest(
    ...
    let candidates = [
        (
            SharedArtifactKind::ContextCards,
            ".ctxpack/cards",
            repo_root.join(".ctxpack/cards"),
        ),
        (
            SharedArtifactKind::BenchmarkReport,
            ".ctxpack/benchmark-report.json",
            repo_root.join(".ctxpack/benchmark-report.json"),
        ),
        (
            SharedArtifactKind::PolicyProfile,
            ".ctxpack/policy-profiles.jsonl",
            repo_root.join(".ctxpack/policy-profiles.jsonl"),
        ),
        (
            SharedArtifactKind::FeedbackSummary,
            ".ctxpack/feedback-summary.json",
            repo_root.join(".ctxpack/feedback-summary.json"),
        ),
        (
            SharedArtifactKind::ProofReport,
            ".ctxpack/proof-report.json",
            repo_root.join(".ctxpack/proof-report.json"),
        ),
        (
            SharedArtifactKind::WorkspaceManifest,
            ".ctxpack/workspace.json",
            repo_root.join(".ctxpack/workspace.json"),
        ),
        (
            SharedArtifactKind::TeamPolicy,
            ".ctxpack/team-policy.json",
...
mod tests {
    ...
    fn shared_manifest_and_policy_are_source_free() {
        ...
        fs::create_dir_all(repo.join(".ctxpack/cards")).unwrap();
        fs::write(
            repo.join(".ctxpack/cards/auth.md"),
        ...
        assert!(!json.contains("CTXPACK_ARTIFACT_SECRET_SENTINEL"));
