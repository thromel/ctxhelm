...
pub fn generate_fallback_cards(
    ...
    let fallback_dir = repo_root.join(".ctxpack").join("fallback");
...
fn render_fallback_guide(report: &ContextCardsReport, target_agent: &str) -> String {
    ...
    output.push_str("# ctxpack Disconnected Fallback\n\n");
    output.push_str(&format!("- Target agent: `{target_agent}`\n"));
    output.push_str(&format!("- Repo ID: `{}`\n", report.repo_id));
    output.push_str("- Generated by: `ctxpack cards fallback`\n");
    ...
    output.push_str("Use this guide only when the local ctxpack MCP server is unavailable. Keep normal daily work inside the coding agent and prefer MCP `prepare_task`/`get_pack` when available.\n\n");
