...
use ctxhelm_core::{CacheStatus, CacheStatusKind, Diagnostic, DiagnosticSeverity};
...
pub fn load_or_refresh_inventory(
    ...
    if repo_root.join(".ctxhelm/eval-history.json").is_file() {
    ...
    match persist_inventory(&inventory) {
        ...
        Err(error) => {
            let cache_diagnostic = diagnostic(
                "cache_write_failed",
                DiagnosticSeverity::Warning,
                "Fresh inventory was returned, but ctxhelm could not persist the cache.",
...
mod tests {
    ...
    fn freshness_detects_content_and_ignore_file_mutations() {
        ...
        fs::create_dir_all(repo.join("src")).unwrap();
        fs::write(repo.join("src/lib.rs"), "pub fn old() {}\n").unwrap();
        fs::write(repo.join(".ctxhelmignore"), "ignored-before.rs\n").unwrap();

        let cached = build_inventory(&repo, &InventoryOptions::default()).unwrap();
        fs::write(repo.join("src/lib.rs"), "pub fn changed() {}\n").unwrap();
        fs::write(repo.join(".ctxhelmignore"), "src/lib.rs\n").unwrap();
        let freshness =
            check_inventory_freshness(&repo, &cached, &InventoryOptions::default()).unwrap();
        ...
        assert!(freshness
            ...
            .any(
                |diagnostic| diagnostic.code == "inventory_stale_ignore_changed"
                    && diagnostic.paths == vec![".ctxhelmignore".to_string()]
            ));
    }
    ...
    fn load_or_refresh_rebuilds_stale_cache_after_file_mutation() {
        ...
        assert_eq!(
            ...
            ctxhelm_core::CacheStatusKind::Rebuilt
        );
        assert!(report
            .diagnostics
            .iter()
    ...
    fn load_or_refresh_trusts_cached_immutable_eval_snapshot_inventory() {
        ...
        fs::create_dir_all(repo.join("src")).unwrap();
        fs::write(repo.join(".ctxhelm/eval-history.json"), "{}\n").unwrap();
    ...
    fn load_or_refresh_returns_inventory_when_cache_write_fails() {
        ...
        assert_eq!(
            report.cache_status.status,
            ctxhelm_core::CacheStatusKind::WriteFailed
        );
        assert!(report
            .diagnostics
            .iter()
