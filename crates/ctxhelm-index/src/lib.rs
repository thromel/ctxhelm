...
pub use storage::{
    initialize_store, inspect_store_schema, list_memory_cards, load_semantic_query_vector_record,
    load_semantic_vector_records, open_store_report, persist_benchmark_run_record,
    persist_context_pack_record, persist_memory_card_records, persist_proof_report_record,
    persist_semantic_vector_records, required_table_names, storage_status_for_path,
    storage_status_for_repo, sync_inventory_to_store, update_memory_card_review_status,
    upsert_semantic_query_vector_record, upsert_semantic_vector_records, vacuum_store,
    ...
    StorageSchemaReport, StorageSemanticIndexReport, StorageSemanticQueryVectorRecord,
    StorageSemanticVectorRecord, StorageStatusReport, StoreConfig, StorePaths,
...
#[cfg(test)]
use ctxhelm_core::{EvalTrace, FeedbackOutcome, FileRole, PackBudget, SessionFeedbackEvent};
#[cfg(test)]
use git::{
...
mod tests {
    ...
    fn inventory_respects_ignores_and_default_exclusions() {
        ...
        fs::write(repo.join(".ctxhelmignore"), "ignored-by-ctxhelm.ts\n").unwrap();
        ...
        fs::write(repo.join("ignored-by-ctxhelm.ts"), "ignored\n").unwrap();
    ...
    #[test]
    fn write_inventory_persists_under_ctxhelm_home() {
        let _guard = env_lock();
        let temp = tempfile::tempdir().unwrap();
    ...
    fn lexical_search_reuses_query_cache_until_inventory_changes() {
        ...
        assert_eq!(
            ...
            ctxhelm_core::CacheStatusKind::Hit
        );
        assert!(second
            .cache_status
            .path
    ...
    fn lexical_search_builds_missing_inventory_and_skips_excluded_files() {
        ...
        fs::create_dir_all(repo.join("src")).unwrap();
        fs::create_dir_all(repo.join("dist")).unwrap();
        fs::write(repo.join(".ctxhelmignore"), "ignored.ts\n").unwrap();
        fs::write(
...
"#,
    ...
    fn related_tests_uses_content_mentions_and_excludes_non_inventory_files() {
        ...
        fs::create_dir_all(repo.join("tests")).unwrap();
        fs::create_dir_all(repo.join("dist")).unwrap();
        fs::write(repo.join(".ctxhelmignore"), "tests/ignored.test.ts\n").unwrap();
        fs::write(
    ...
    fn co_change_hints_use_eval_history_sidecar_when_snapshot_has_no_git() {
        ...
        run_git(
            &source_repo,
            &["config", "user.email", "ctxhelm@example.com"],
        );
        run_git(&source_repo, &["config", "user.name", "ctxhelm"]);
        fs::write(
    ...
    fn eval_traces_append_and_list_without_source_text() {
        ...
        let first = EvalTrace {
            ...
            task_type: ctxhelm_core::TaskType::BugFix,
            pack_id: None,
