...
pub fn default_workspace_manifest_path(
    ...
    Ok(root.join(".ctxhelm").join("workspace.json"))
}
...
mod tests {
    ...
    fn git_repo(path: &Path) {
        ...
        fs::write(path.join(".env"), "CTXHELM_WORKSPACE_SOURCE_SENTINEL\n").unwrap();
    ...
    fn workspace_status_reports_multiple_repos_without_source_text() {
        ...
        let manifest_path = workspace.join(".ctxhelm/workspace.json");
        ...
        assert!(!json.contains("CTXHELM_WORKSPACE_SOURCE_SENTINEL"));
    ...
    fn workspace_status_diagnoses_missing_and_duplicate_repos() {
        ...
        let api = workspace.join("api");
        git_repo(&api);
        fs::create_dir_all(workspace.join(".ctxhelm")).unwrap();
        let manifest = WorkspaceManifest {
            schema_version: WORKSPACE_MANIFEST_SCHEMA_VERSION,
            workspace_id: None,
            repos: vec![
