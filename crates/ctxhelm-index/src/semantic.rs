...
pub const DEFAULT_SEMANTIC_MODEL: &str = "ctxhelm-local-hash-v1";
...
pub fn semantic_document_report(
    ...
    let precision_status = if options.include_dependencies {
        ...
        precision_status_from_dependencies(&repo_root, &[], &[])
    };

    let mut semantic_files = inventory
        .files
        .iter()
    ...
    semantic_files.truncate(options.limit.max(1));

    let mut documents = Vec::new();
    for file in semantic_files.iter() {
        ...
        if options.include_related_tests && matches!(file.role, FileRole::Source) {
...
pub fn semantic_search_report(
    ...
    let skipped_cache_status = CacheStatus {
        status: ctxhelm_core::CacheStatusKind::Skipped,
        path: None,
        diagnostics: Vec::new(),
    };
    if !options.enabled {
        ...
        return Ok(SemanticSearchReport {
            results: Vec::new(),
            diagnostics,
            cache_status: skipped_cache_status,
            privacy_status: PrivacyStatus::local_only(),
            provider,
        });
    }
    ...
    let mut file_vectors = candidate_documents
        ...
        .collect::<Vec<_>>();
    let reused_vector_count = file_vectors
        .iter()
    ...
    if reused_vector_count > 0 {
        diagnostics.push(semantic_vector_storage_reused_diagnostic(
            reused_vector_count,
            missing_indices.len(),
        ));
    }
        ...
        .zip(file_vectors.into_iter())
    {
        ...
        results.push(SemanticSearchResult {
            ...
            score,
            reason: semantic_match_reason(&provider, &matched_facets, exact_boost),
...
pub fn semantic_vector_records(
    ...
    let document_report = semantic_document_report(
        &repo_root,
        &SemanticDocumentOptions {
            limit: semantic_vector_document_limit(options.limit),
            query: None,
            include_symbols: false,
            include_dependencies: false,
            include_related_tests: false,
...
fn semantic_search_document_limit(provider: &SemanticProviderConfig, result_limit: usize) -> usize {
    let result_limit = result_limit.max(1);
    let candidate_floor = if provider.provider == LOCAL_FASTEMBED_PROVIDER {
        local_fastembed_document_prefilter_limit()
    } else {
        64
    };
    candidate_floor
        .max(result_limit.saturating_mul(16))
        .min(512)
}

fn semantic_vector_document_limit(result_limit: usize) -> usize {
    if result_limit == usize::MAX {
        usize::MAX
    } else {
        result_limit.max(1)
    }
...
fn semantic_document_storage_key(path: &str, safe_hash: &str) -> String {
    format!("{path}\n{safe_hash}")
}
...
fn stored_semantic_vectors_by_document(
    ...
    let records = match load_semantic_vector_records(
        ...
        Err(error) => {
            diagnostics.push(Diagnostic {
                code: "semantic_vector_storage_unavailable".to_string(),
                severity: DiagnosticSeverity::Warning,
                message: format!("Could not read persisted semantic vectors: {error}"),
                paths: Vec::new(),
                count: 0,
            });
            return BTreeMap::new();
...
fn semantic_vector_storage_reused_diagnostic(reused: usize, embedded: usize) -> Diagnostic {
    Diagnostic {
        code: "semantic_vector_storage_reused".to_string(),
        severity: DiagnosticSeverity::Info,
        message: format!(
            "Semantic search reused {reused} persisted source-free vector(s) and embedded {embedded} candidate document(s)."
        ),
        paths: Vec::new(),
        count: reused,
    }
...
fn local_fastembed_document_prefilter_limit() -> usize {
    std::env::var("CTXHELM_FASTEMBED_DOCUMENT_LIMIT")
        .ok()
...
fn semantic_file_prefilter_score(
    file: &FileInventoryEntry,
    terms: &[String],
    symbols_by_path: &BTreeMap<String, Vec<CodeSymbol>>,
    edges_by_path: &BTreeMap<String, Vec<DependencyEdge>>,
) -> f32 {
    if terms.is_empty() {
        return 0.0;
    }
    let path = file.path.to_ascii_lowercase();
    let language = file.language.as_deref().unwrap_or("").to_ascii_lowercase();
    let role = format!("{:?}", file.role).to_ascii_lowercase();
    let mut score = 0.0;
    for term in terms {
        if path.contains(term) {
            score += 4.0;
        ...
        for symbol in symbols_by_path
            ...
            .take(24)
        {
            let text = format!("{} {}", symbol.name, symbol.signature).to_ascii_lowercase();
            if text.contains(term) {
                score += if symbol.exported { 2.0 } else { 1.4 };
            }
        }
        for edge in edges_by_path.get(&file.path).into_iter().flatten().take(24) {
            let text = format!(
                "{} {} {} {}",
                edge.kind, edge.reason, edge.source_path, edge.target_path
            )
            .to_ascii_lowercase();
            if text.contains(term) {
                score += edge.confidence.clamp(0.1, 1.0);
            }
        }
    }
    score
...
fn semantic_match_reason(
    ...
    let boost = if exact_boost > 0.0 {
        format!("; exact metadata boost {exact_boost:.3}")
    } else {
        String::new()
    };
    format!(
        "local semantic similarity via {} {} over source-free facets: {}{}",
        provider.provider, provider.model, facet_summary, boost
    )
}
...
fn semantic_provider_unavailable_diagnostic(provider: &SemanticProviderConfig) -> Diagnostic {
    let message = if provider.provider == LOCAL_FASTEMBED_PROVIDER {
        "Semantic provider local_fastembed requires the ctxhelm-index local-embeddings feature and remains opt-in; no cloud provider was used."
    } else {
        "Semantic provider is unsupported by this ctxhelm build; no cloud provider was used."
    };
    Diagnostic {
        code: "semantic_provider_unavailable".to_string(),
        severity: DiagnosticSeverity::Warning,
...
fn local_fastembed_model_cache_dir() -> std::path::PathBuf {
    std::env::var("CTXHELM_FASTEMBED_CACHE_DIR")
        ...
        .unwrap_or_else(|_| {
            std::env::current_dir()
                ...
                .map(|root| root.join(".ctxhelm").join("cache").join("fastembed"))
                .unwrap_or_else(|| {
                    crate::inventory::ctxhelm_home()
                        .join("cache")
                        .join("fastembed")
                })
        })
...
fn local_fastembed_vectors(
    ...
    Err(format!(
        "Semantic provider {} requires the ctxhelm-index local-embeddings feature; no cloud provider was used.",
        provider.provider
    ))
}
...
mod tests {
    ...
    #[test]
    fn semantic_search_boosts_exact_source_free_path_metadata() {
        let temp = tempfile::tempdir().unwrap();
        let repo = temp.path();
        fs::create_dir(repo.join(".git")).unwrap();
        fs::create_dir_all(repo.join("src/refactor")).unwrap();
        fs::write(
            repo.join("src/refactor/type_script_visitor.rs"),
            "pub fn unrelated() {}\n",
        )
        .unwrap();
        fs::write(
            repo.join("src/refactor/generic_modifier.rs"),
            "pub fn type_script() {}\n",
        )
        .unwrap();
        ...
        assert_eq!(
            ...
            "src/refactor/type_script_visitor.rs"
        );
        assert!(report.results[0].reason.contains("exact metadata boost"));
        assert!(report.results[0]
            .matched_facets
            .iter()
            .any(|facet| facet.label == "path"));
    }

    #[test]
    fn semantic_search_reuses_persisted_source_free_vectors() {
        ...
        let document_report = semantic_document_report(
            ...
            &SemanticDocumentOptions {
                limit: 10,
                query: Some("payment webhook".to_string()),
                include_symbols: false,
                include_dependencies: false,
                include_related_tests: false,
            },
        ...
        .unwrap();
        let payment_document = document_report
            .documents
            .iter()
            .find(|document| document.path == "src/payments.ts")
            .unwrap();
    ...
    fn semantic_documents_are_source_free_and_precision_enriched() {
        ...
        .unwrap();
        fs::write(
            repo.join(".ctxhelm/precision-edges.json"),
            r#"{
  "schemaVersion": 1,
