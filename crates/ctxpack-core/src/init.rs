...
For non-trivial code changes, call the ctxpack MCP tool `prepare_task` before planning edits when MCP is configured. Pass the active repository path as `repo` when the agent knows it. Otherwise, run `ctxpack prepare-task --repo <repo>`.
...
1. Call the ctxpack MCP tool `prepare_task` when available, passing the active repository path as `repo` when known, or run `ctxpack prepare-task --repo <repo>`.
...
pub const OPENCODE_SNIPPET: &str = r#"{
  ...
  "ctxpackNote": "Registers ctxpack as a local read-only MCP context server. Call prepare_task with the active repository path as `repo`, read files natively, and use get_pack progressively only when needed. Pack resources are same-session only; after reconnect or restart call get_pack.",
...
Copy/paste this local stdio MCP server setup yourself; ctxpack does not apply it automatically:
...
If an agent cannot spawn `ctxpack`, replace the command with the absolute path from `which ctxpack`. This command does not mutate global Codex config automatically.
"#;
...
pub fn run_setup_check(
    ...
    for adapter in &options.adapters {
        for (path, _) in adapter_files(*adapter) {
            let kind = match path {
                ".ctxpack/adapters/claude-mcp.json" => SetupFileKind::Json(300),
                ".ctxpack/adapters/opencode.jsonc.snippet" => SetupFileKind::Json(900),
                ".cursor/rules/ctxpack.mdc" => SetupFileKind::Guidance(1_400),
                ".claude/commands/ctxpack-bugfix.md" => SetupFileKind::Guidance(1_400),
    ...
    items.push(SetupCheckItem {
        name: "ctxpack binary path guidance".to_string(),
        status: SetupCheckStatus::Warn,
        detail: "Run `ctxpack --version`; if an agent cannot find ctxpack on PATH, use the absolute path from `which ctxpack` in that agent's explicit configuration.".to_string(),
    });
...
fn validate_thin_guidance(
    ...
    if relative_path == "AGENTS.md" {
        for marker in [AGENTS_SECTION_START, AGENTS_SECTION_END] {
            if !content.contains(marker) {
                return Err(format!("AGENTS.md is missing ctxpack marker `{marker}`"));
            }
        }
    }
    ...
    for required in ["prepare_task", "repo", "native", "get_pack"] {
        if !content.contains(required) {
            return Err(format!(
                "generated guidance is missing required ctxpack term `{required}`"
            ));
        }
    }
...
fn init_next_steps() -> Vec<InitNextStep> {
    vec![
        InitNextStep {
            ...
            command: "ctxpack --version && ctxpack --help".to_string(),
            detail: "Confirm the ctxpack binary is the expected release and exposes the installed CLI surface."
                .to_string(),
        },
        InitNextStep {
            label: "Validate setup artifacts".to_string(),
            command: "ctxpack setup-check --repo <repo>".to_string(),
        ...
        InitNextStep {
            ...
            command: "codex mcp add ctxpack -- ctxpack serve-mcp".to_string(),
            detail: "Copy/paste or merge the generated guidance yourself; ctxpack does not mutate global agent config."
                .to_string(),
        },
        InitNextStep {
...
mod tests {
    ...
    fn codex_setup_is_guidance_only() {
        ...
        assert!(CODEX_MCP_SETUP.contains("codex mcp add ctxpack -- ctxpack serve-mcp"));
...
mod writer_tests {
    ...
    fn init_is_idempotent() {
        ...
        assert_file_action(&second, ".ctxpack/ctxpack.toml", InitAction::Unchanged);
        assert_file_action(&second, "AGENTS.md", InitAction::Unchanged);
        assert_file_action(&second, ".cursor/rules/ctxpack.mdc", InitAction::Unchanged);
        assert_file_action(
            &second,
            ".ctxpack/adapters/opencode.jsonc.snippet",
            InitAction::Unchanged,
        ...
        assert_file_action(
            &second,
            ".ctxpack/adapters/claude-mcp.json",
            InitAction::Skipped,
        );
    }
    ...
    fn init_report_records_skipped_unrequested_adapters() {
        ...
        assert_file_action(&report, ".ctxpack/ctxpack.toml", InitAction::Created);
        assert_file_action(&report, "AGENTS.md", InitAction::Created);
        assert_file_action(&report, ".cursor/rules/ctxpack.mdc", InitAction::Skipped);
        assert_file_action(
            &report,
            ".claude/commands/ctxpack-bugfix.md",
            InitAction::Skipped,
        ...
        assert_file_action(&second, ".ctxpack/ctxpack.toml", InitAction::Unchanged);
        assert_file_action(&second, "AGENTS.md", InitAction::Unchanged);
        assert_file_action(&second, ".cursor/rules/ctxpack.mdc", InitAction::Skipped);
    }
    ...
    fn init_report_records_requested_adapters_as_written() {
        ...
        assert_file_action(&report, ".cursor/rules/ctxpack.mdc", InitAction::Created);
        assert_file_action(
            &report,
            ".claude/commands/ctxpack-bugfix.md",
            InitAction::Created,
    ...
    fn init_report_next_steps_name_first_pack_ladder() {
        ...
        assert!(rendered.contains("ctxpack --version"));
        assert!(rendered.contains("ctxpack --help"));
        assert!(rendered.contains("ctxpack setup-check --repo"));
...
mod setup_check_tests {
    ...
    fn setup_check_passes_generated_artifacts() {
        ...
        assert_item_status(&report, ".ctxpack/ctxpack.toml", SetupCheckStatus::Pass);
        assert_item_status(&report, ".cursor/rules/ctxpack.mdc", SetupCheckStatus::Pass);
        assert_item_status(
            &report,
            ".claude/commands/ctxpack-bugfix.md",
    ...
    fn setup_check_fails_missing_expected_file() {
        ...
        run_init(temp.path(), &options).unwrap();
        std::fs::remove_file(temp.path().join(".cursor/rules/ctxpack.mdc")).unwrap();

        let report = run_setup_check(temp.path(), &options).unwrap();

        assert!(!report.passed);
        assert_item_status(&report, ".cursor/rules/ctxpack.mdc", SetupCheckStatus::Fail);
    }
    ...
    fn setup_check_validates_adapter_json() {
        ...
        run_init(temp.path(), &options).unwrap();
        std::fs::write(
            temp.path().join(".ctxpack/adapters/claude-mcp.json"),
            "{not-json",
        )
        .unwrap();
    ...
    fn setup_check_flags_large_static_context_guidance() {
        ...
        run_init(temp.path(), &options).unwrap();
        std::fs::write(
            temp.path().join(".cursor/rules/ctxpack.mdc"),
            "Repository map:\nsource snippets\n",
        )
        .unwrap();
    ...
    fn setup_check_includes_absolute_binary_path_guidance() {
        ...
        assert!(rendered.contains("ctxpack --version"));
        assert!(rendered.contains("which ctxpack"));
