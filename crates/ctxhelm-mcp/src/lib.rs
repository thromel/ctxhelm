...
mod tests {
    ...
    fn initialize_public_capabilities_are_stable() {
        ...
        assert_eq!(response["result"]["serverInfo"]["name"], "ctxhelm");
        assert_eq!(
    ...
    fn resources_list_exposes_repo_resources() {
        ...
        assert_eq!(resources[0]["uri"], "ctxhelm://repo/summary");
        assert_eq!(resources[1]["uri"], "ctxhelm://repo/test-map");
        assert_eq!(resources[2]["uri"], "ctxhelm://repo/dependency-graph");
        assert_eq!(resources[3]["uri"], "ctxhelm://repo/memory");
        assert_eq!(resources[4]["uri"], "ctxhelm://repo/context-areas");
        assert_eq!(resources[5]["uri"], "ctxhelm://workspace/status");
        assert_eq!(resources[6]["uri"], "ctxhelm://workspace/shared-artifacts");
        assert_eq!(resources[7]["uri"], "ctxhelm://pack/guide");
    }
    ...
    fn resources_public_uri_shapes_are_stable() {
        ...
        std::env::set_var("CTXHELM_HOME", &repo.home);
        let cwd = std::env::current_dir().unwrap();
        std::env::set_current_dir(&repo.repo).unwrap();
        ctxhelm_index::write_workspace_manifest(
            &repo.repo,
            &ctxhelm_core::WorkspaceManifest {
                schema_version: ctxhelm_index::WORKSPACE_MANIFEST_SCHEMA_VERSION,
                workspace_id: None,
                repos: vec![ctxhelm_core::WorkspaceRepo {
                    id: None,
                    path: ".".to_string(),
        ...
        .unwrap();
        ctxhelm_index::export_shared_artifact_manifest(&repo.repo).unwrap();
        ...
        let summary = handle_line(
            r#"{"jsonrpc":"2.0","id":30,"method":"resources/read","params":{"uri":"ctxhelm://repo/summary"}}"#,
        )
        .unwrap();
        let test_map = handle_line(
            r#"{"jsonrpc":"2.0","id":31,"method":"resources/read","params":{"uri":"ctxhelm://repo/test-map"}}"#,
        )
        .unwrap();
        let dependency_graph = handle_line(
            r#"{"jsonrpc":"2.0","id":32,"method":"resources/read","params":{"uri":"ctxhelm://repo/dependency-graph"}}"#,
        )
        .unwrap();
        let guide = handle_line(
            r#"{"jsonrpc":"2.0","id":33,"method":"resources/read","params":{"uri":"ctxhelm://pack/guide"}}"#,
        )
        .unwrap();
        let memory = handle_line(
            r#"{"jsonrpc":"2.0","id":36,"method":"resources/read","params":{"uri":"ctxhelm://repo/memory"}}"#,
        )
        .unwrap();
        let context_areas = handle_line(
            r#"{"jsonrpc":"2.0","id":39,"method":"resources/read","params":{"uri":"ctxhelm://repo/context-areas"}}"#,
        )
        .unwrap();
        let context_area = handle_line(
            r#"{"jsonrpc":"2.0","id":40,"method":"resources/read","params":{"uri":"ctxhelm://repo/context-area/src%2Fauth"}}"#,
        )
        .unwrap();
        let workspace = handle_line(
            r#"{"jsonrpc":"2.0","id":37,"method":"resources/read","params":{"uri":"ctxhelm://workspace/status"}}"#,
        )
        .unwrap();
        let shared_artifacts = handle_line(
            r#"{"jsonrpc":"2.0","id":38,"method":"resources/read","params":{"uri":"ctxhelm://workspace/shared-artifacts"}}"#,
        )
        .unwrap();
        let file = handle_line(
            r#"{"jsonrpc":"2.0","id":34,"method":"resources/read","params":{"uri":"ctxhelm://file/src/auth/session.ts?lines=1-2"}}"#,
        )
        .unwrap();
        let symbol = handle_line(
            r#"{"jsonrpc":"2.0","id":35,"method":"resources/read","params":{"uri":"ctxhelm://symbol/requireSession"}}"#,
        )
        .unwrap();
        ...
        assert!(context_areas["result"]["contents"][0]["text"]
            .as_str()
            .unwrap()
            .contains("ctxhelm://repo/context-area/src%2Fauth"));
    ...
    fn diagnostics_prepare_task_and_get_pack_include_trace_write_failures() {
        let _guard = env_lock();
        let repo = fixture_repo();
        let blocked_home = repo._temp.path().join("ctxhelm-home-file");
        fs::write(&blocked_home, "not a directory\n").unwrap();
        std::env::set_var("CTXHELM_HOME", &blocked_home);
    ...
    fn diagnostics_search_reports_stale_cache_rebuild() {
        ...
        let repo = fixture_repo();
        std::env::set_var("CTXHELM_HOME", &repo.home);
        ctxhelm_index::write_inventory(&repo.repo, &ctxhelm_index::InventoryOptions::default())
            .unwrap();
        fs::write(
    ...
    fn file_resource_revalidates_against_current_safe_inventory() {
        ...
        ctxhelm_index::write_inventory(&repo.repo, &ctxhelm_index::InventoryOptions::default())
            .unwrap();
        fs::write(repo.repo.join(".ctxhelmignore"), "src/auth/session.ts\n").unwrap();

        let response = handle_line(
            r#"{"jsonrpc":"2.0","id":78,"method":"resources/read","params":{"uri":"ctxhelm://file/src/auth/session.ts?lines=1-2"}}"#,
        )
        .unwrap();
    ...
    fn pack_resources_are_session_scoped_characterization() {
        ...
        let guide = handle_line(
            r#"{"jsonrpc":"2.0","id":49,"method":"resources/read","params":{"uri":"ctxhelm://pack/guide"}}"#,
        )
        .unwrap();
    ...
    fn resource_read_returns_repo_summary_and_test_map() {
        ...
        let summary = handle_line(
            r#"{"jsonrpc":"2.0","id":10,"method":"resources/read","params":{"uri":"ctxhelm://repo/summary"}}"#,
        )
        .unwrap();
        let test_map = handle_line(
            r#"{"jsonrpc":"2.0","id":11,"method":"resources/read","params":{"uri":"ctxhelm://repo/test-map"}}"#,
        )
        .unwrap();
        let dependency_graph = handle_line(
            r#"{"jsonrpc":"2.0","id":17,"method":"resources/read","params":{"uri":"ctxhelm://repo/dependency-graph"}}"#,
        )
        .unwrap();
    ...
    fn repo_resources_use_last_explicit_tool_repo_when_server_cwd_is_not_repo() {
        ...
        let context_area = handle_line(
            r#"{"jsonrpc":"2.0","id":602,"method":"resources/read","params":{"uri":"ctxhelm://repo/context-area/src%2Fauth"}}"#,
        )
        .unwrap();
    ...
    fn resource_test_map_uses_package_aware_commands() {
        ...
        let test_map = handle_line(
            r#"{"jsonrpc":"2.0","id":20,"method":"resources/read","params":{"uri":"ctxhelm://repo/test-map"}}"#,
        )
        .unwrap();
    ...
    fn resource_read_returns_safe_file_slice_and_symbol_results() {
        ...
        let file = handle_line(
            r#"{"jsonrpc":"2.0","id":12,"method":"resources/read","params":{"uri":"ctxhelm://file/src/auth/session.ts?lines=1-2"}}"#,
        )
        .unwrap();
        let symbol = handle_line(
            r#"{"jsonrpc":"2.0","id":13,"method":"resources/read","params":{"uri":"ctxhelm://symbol/requireSession"}}"#,
        )
        .unwrap();
    ...
    fn prompts_get_returns_agent_workflow_instruction() {
        ...
        assert!(text.contains("ctxhelm.prepare_task"));
    ...
    fn semantic_mcp_calls_are_additive_and_source_free() {
        ...
        let search = handle_line(&format!(
            r#"{{"jsonrpc":"2.0","id":86,"method":"tools/call","params":{{"name":"search","arguments":{{"query":"auth required","repo":"{}","limit":10,"semantic":true,"semanticProvider":"local_hash","semanticModel":"ctxhelm-mcp-test","semanticDimensions":128}}}}}}"#,
            repo.repo.display()
        ))
        .unwrap();
        ...
        let prepare = handle_line(&format!(
            r#"{{"jsonrpc":"2.0","id":87,"method":"tools/call","params":{{"name":"prepare_task","arguments":{{"task":"fix auth required flow","repo":"{}","mode":"bug_fix","semantic":true,"semanticProvider":"local_hash","semanticModel":"ctxhelm-mcp-test","semanticDimensions":128,"recordTrace":false}}}}}}"#,
            repo.repo.display()
        ))
        .unwrap();
        ...
        let pack = handle_line(&format!(
            r#"{{"jsonrpc":"2.0","id":88,"method":"tools/call","params":{{"name":"get_pack","arguments":{{"task":"fix auth required flow","repo":"{}","mode":"bug_fix","semantic":true,"semanticProvider":"ctxhelm_mcp_unknown_provider","recordTrace":false,"format":"json"}}}}}}"#,
            repo.repo.display()
        ))
        .unwrap();
        ...
        assert!(decisions.iter().any(|decision| {
            decision["provider"] == "ctxhelm_mcp_unknown_provider"
    ...
    fn json_rpc_error_codes_are_stable() {
        ...
        let unsupported_resource = handle_line(
            r#"{"jsonrpc":"2.0","id":62,"method":"resources/read","params":{"uri":"ctxhelm://unknown/resource"}}"#,
        )
        .unwrap();
