...
Renders packaging/homebrew/ctxpack.rb.template into a concrete formula file.
...
if [[ ! "$url" =~ ^https://github[.]com/thromel/ctxpack/releases/download/v${version}/ctxpack-v${version}-[^/]+[.]tar[.]gz$ ]]; then
...
template="$repo_root/packaging/homebrew/ctxpack.rb.template"
if [[ ! -f "$template" ]]; then
...
if "CTXPACK_" in text:
    raise SystemExit("unrendered CTXPACK placeholder remains")
