...
if path:
    ...
    print("ctxpack OpenCode setup evidence: " + payload)
PY
}
...
CTXPACK_HOME="$home" "$ctxpack_bin" init --repo "$repo" --opencode >/dev/null
CTXPACK_HOME="$home" "$ctxpack_bin" setup-check --repo "$repo" --opencode >/dev/null

CTXPACK_BIN="$ctxpack_bin" \
  CTXPACK_ROOT="$ctxpack_root" \
  CTXPACK_SMOKE_REPO="$repo" \
  CTXPACK_SMOKE_TASK="verify OpenCode ctxpack setup proof" \
...
if [[ -n "${CTXPACK_REAL_CLIENT_EVIDENCE_DIR:-}" ]]; then
  evidence_path="${CTXPACK_REAL_CLIENT_EVIDENCE_DIR}/opencode-setup-evidence.json"
fi
write_evidence "$evidence_path"
echo "ctxpack OpenCode setup smoke passed"
