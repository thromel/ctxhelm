...
if path:
    ...
    print("ctxhelm OpenCode setup evidence: " + payload)
PY
}
...
CTXHELM_HOME="$home" "$ctxhelm_bin" init --repo "$repo" --opencode >/dev/null
CTXHELM_HOME="$home" "$ctxhelm_bin" setup-check --repo "$repo" --opencode >/dev/null

CTXHELM_BIN="$ctxhelm_bin" \
  CTXHELM_ROOT="$ctxhelm_root" \
  CTXHELM_SMOKE_REPO="$repo" \
  CTXHELM_SMOKE_TASK="verify OpenCode ctxhelm setup proof" \
...
if [[ -n "${CTXHELM_REAL_CLIENT_EVIDENCE_DIR:-}" ]]; then
  evidence_path="${CTXHELM_REAL_CLIENT_EVIDENCE_DIR}/opencode-setup-evidence.json"
fi
write_evidence "$evidence_path"
echo "ctxhelm OpenCode setup smoke passed"
