...
require_real="${CTXHELM_REQUIRE_OPENCODE_REAL_CLIENT:-${CTXHELM_REQUIRE_REAL_CLIENT:-0}}"
run_real="${CTXHELM_RUN_OPENCODE_REAL_CLIENT:-${CTXHELM_RUN_REAL_CLIENT:-0}}"
...
def request_log_summary(log_path, expected_repo):
    ...
    return {
        "requestEvidenceSchemaVersion": "ctxhelm-opencode-real-client-evidence-v1",
...
evidence = {
    ...
    "proofBoundary": "OpenCode real-client proof is source-free and passes only when server-side instrumentation records prepare_task and get_pack with the explicit repo.",
}
...
if path:
    ...
    print("ctxhelm OpenCode real-client evidence: " + payload)
PY
}
...
fail_or_skip() {
  ...
  if [[ -n "${CTXHELM_REAL_CLIENT_EVIDENCE_DIR:-}" ]]; then
    write_evidence_json "${CTXHELM_REAL_CLIENT_EVIDENCE_DIR}/opencode-real-client-evidence.json" "skipped" "$reason" "$request_log_path" "$client_status"
  fi
  if [[ "$require_real" == "1" ]]; then
    echo "ctxhelm OpenCode real-client smoke failed: $reason" >&2
    exit 1
  fi
  echo "ctxhelm OpenCode real-client smoke skipped: $reason"
  exit 0
}
...
echo "ctxhelm OpenCode real-client smoke: running deterministic protocol gate"
...
if [[ "$require_real" != "1" && "$run_real" != "1" ]]; then
  fail_or_skip "set CTXHELM_RUN_OPENCODE_REAL_CLIENT=1 or CTXHELM_REQUIRE_OPENCODE_REAL_CLIENT=1 after protocol gate passed"
fi
if ! command -v opencode >/dev/null 2>&1; then
  fail_or_skip "opencode is not installed"
fi

client_version="$(opencode --version 2>&1 | head -n 1)"
request_log="$work_dir/ctxhelm-mcp-requests.jsonl"
events="$work_dir/opencode-events.jsonl"
stderr_log="$work_dir/opencode-stderr.log"
...
python3 - "$opencode_config" "$server_wrapper" <<'PY'
import json
import sys
...
OPENCODE_CONFIG="$opencode_config" \
  ...
  --dir "$repo" \
  --format json \
  --title "ctxhelm real-client smoke" \
...
if [[ "$evidence_found" == "1" ]]; then
  evidence_path=""
  if [[ -n "${CTXHELM_REAL_CLIENT_EVIDENCE_DIR:-}" ]]; then
    evidence_path="${CTXHELM_REAL_CLIENT_EVIDENCE_DIR}/opencode-real-client-evidence.json"
  fi
  write_evidence_json "$evidence_path" "passed" "" "$request_log" "$client_status"
  echo "ctxhelm OpenCode real-client smoke passed: server-side instrumentation recorded prepare_task and get_pack with repo=$repo"
  exit 0
fi

if [[ -s "$stderr_log" ]]; then
  echo "ctxhelm OpenCode real-client smoke diagnostic stderr:" >&2
  tail -n 40 "$stderr_log" >&2
fi
fail_or_skip "opencode did not produce machine-checkable prepare_task/get_pack evidence" "$request_log" "$client_status"
