...
payload = {
    ...
    "distributionDecision": {
        ...
        "homebrewFormula": "deferred",
        "cratesIo": "deferred",
        "signedInstaller": "deferred",
        "selfUpdate": "not_implemented",
        "reason": "v1.1.1 production candidate is archive-first; package-manager publication, signed installers, and self-update remain future work.",
    },
    "knownLimitations": [
        "Cursor and OpenCode real-client proof is not claimed for v1.1.1.",
...
for deferred in ["homebrewFormula", "cratesIo", "signedInstaller"]:
    if distribution.get(deferred) != "deferred":
        raise SystemExit(f"{deferred} must be deferred for v1.1.1")
