...
payload = {
    ...
    "distributionDecision": {
        ...
        "reason": "v1.1.6 production candidate supports local archives and the Apple Silicon Homebrew tap; crates.io publication, signed installers, and self-update remain future work.",
    },
    "knownLimitations": [
        "Cursor and OpenCode real-client proof is not claimed for v1.1.6.",
...
if distribution.get("homebrewFormula") != expected_homebrew:
    raise SystemExit(f"homebrewFormula must be {expected_homebrew} for v1.1.6")
for deferred in ["cratesIo", "signedInstaller"]:
    if distribution.get(deferred) != "deferred":
        raise SystemExit(f"{deferred} must be deferred for v1.1.6")
