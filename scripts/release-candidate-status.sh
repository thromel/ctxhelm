...
payload = {
    ...
    "distributionDecision": {
        ...
        "reason": "v1.1.0 production candidate is archive-first; package-manager publication, signed installers, and self-update remain future work.",
    },
    "knownLimitations": [
...
for deferred in ["homebrewFormula", "cratesIo", "signedInstaller"]:
    if distribution.get(deferred) != "deferred":
        raise SystemExit(f"{deferred} must be deferred for v1.1.0")
