...
payload = {
    ...
    "distributionDecision": {
        ...
        "reason": "v2.4.2 production candidate supports local archives, the Apple Silicon Homebrew tap, and a tag-publishing multi-platform archive workflow; crates.io publication, signed installers, and self-update remain future work.",
    },
    "knownLimitations": [
...
if distribution.get("publishedAdditionalPlatformArchives") != expected_platform_assets:
    raise SystemExit(
        f"publishedAdditionalPlatformArchives must be {expected_platform_assets} for v2.4.2"
    )
expected_homebrew = "ready" if payload.get("status") == "ready" else "deferred"
if distribution.get("homebrewFormula") != expected_homebrew:
    raise SystemExit(f"homebrewFormula must be {expected_homebrew} for v2.4.2")
for deferred in ["cratesIo", "signedInstaller"]:
    if distribution.get(deferred) != "deferred":
        raise SystemExit(f"{deferred} must be deferred for v2.4.2")
