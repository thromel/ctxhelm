    ...
    cat >"$proof_summary" <<'JSON'
{
  ...
  "releaseArchive": {
    "name": "ctxhelm-v1.1.11-aarch64-apple-darwin.tar.gz",
...
mkdir -p "$assets_dir"
printf 'archive\n' >"$assets_dir/ctxhelm-v1.1.11-test.tar.gz"
printf 'manifest\n' >"$assets_dir/ctxhelm-v1.1.11-test.manifest.json"
...
archive_digest = hashlib.sha256((assets / "ctxhelm-v1.1.11-test.tar.gz").read_bytes()).hexdigest()
manifest_digest = hashlib.sha256((assets / "ctxhelm-v1.1.11-test.manifest.json").read_bytes()).hexdigest()
payload = {
    "assets": [
        {
            "digest": f"sha256:{archive_digest}",
            "name": "ctxhelm-v1.1.11-test.tar.gz",
            "state": "uploaded",
        },
        {
            "digest": f"sha256:{manifest_digest}",
            "name": "ctxhelm-v1.1.11-test.manifest.json",
            "state": "uploaded",
        },
    ],
    "isDraft": False,
...
printf 'artifact placeholder\n' >"$candidate_dir/ctxhelm-v1.1.11-aarch64-apple-darwin.tar.gz"
