...
DEFAULT_FIXTURE_ROOT="$(cd "$ROOT_DIR/.." && pwd)/ctxpack-proof-fixtures"
FIXTURE_ROOT="${CTXPACK_PROOF_FIXTURE_ROOT:-$DEFAULT_FIXTURE_ROOT}"
...
verischema_source="${CTXPACK_VERISCHEMA_SOURCE:-git@github.com:thromel/VeriSchema.git}"
if [[ -d "$(cd "$ROOT_DIR/.." && pwd)/VeriSchema/.git" ]]; then
  verischema_source="${CTXPACK_VERISCHEMA_SOURCE:-$(cd "$ROOT_DIR/.." && pwd)/VeriSchema}"
fi

ensure_fixture "RefactoringMiner" "git@github.com:thromel/RefactoringMiner.git" "949bddcd3509a805f5e3bcc55fcdb71a691b0dac"
ensure_fixture "ctxpack" "git@github.com:thromel/ctxpack.git" "7f439fa1b1f2784aed6425b695d7c35944ea7955"
